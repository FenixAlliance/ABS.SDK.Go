...
> PricingRuleDtoEnvelope GetPricingRuleById(ctx, pricingRuleId).TenantId(tenantId).ApiVersion(apiVersion).XApiVersion(xApiVersion).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.PricingRulesAPI.GetPricingRuleById(context.Background(), pricingRuleId).TenantId(tenantId).ApiVersion(apiVersion).XApiVersion(xApiVersion).Execute()
	if err != nil {
...
func main() {
	...
	// response from `GetPricingRulesCountAsync`: Int32Envelope
	fmt.Fprintf(os.Stdout, "Response from `PricingRulesAPI.GetPricingRulesCountAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetPricingRulesCountAsyncRequest struct via the builder pattern

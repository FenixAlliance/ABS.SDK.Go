/*
AccountingService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0.0
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
)

// checks if the AccountDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AccountDto{}

// AccountDto struct for AccountDto
type AccountDto struct {
	Id NullableString `json:"id,omitempty"`
	Timestamp NullableTime `json:"timestamp,omitempty"`
	Group *bool `json:"group,omitempty"`
	Frozen *bool `json:"frozen,omitempty"`
	Name NullableString `json:"name,omitempty"`
	Code NullableString `json:"code,omitempty"`
	Path NullableString `json:"path,omitempty"`
	Title NullableString `json:"title,omitempty"`
	Prefix NullableString `json:"prefix,omitempty"`
	Balance *float64 `json:"balance,omitempty"`
	CurrencyId NullableString `json:"currencyId,omitempty"`
	ContactId NullableString `json:"contactId,omitempty"`
	AccountType NullableString `json:"accountType,omitempty"`
	QualifiedName NullableString `json:"qualifiedName,omitempty"`
	AccountTypeId NullableString `json:"accountTypeId,omitempty"`
	DebitsBalance *float64 `json:"debitsBalance,omitempty"`
	CreditsBalance *float64 `json:"creditsBalance,omitempty"`
	BalanceInUsd *float64 `json:"balanceInUsd,omitempty"`
	DebitsBalanceInUsd *float64 `json:"debitsBalanceInUsd,omitempty"`
	CreditsBalanceInUsd *float64 `json:"creditsBalanceInUsd,omitempty"`
	ForexRate *float64 `json:"forexRate,omitempty"`
	ParentAccountId NullableString `json:"parentAccountId,omitempty"`
	TenantId NullableString `json:"tenantId,omitempty"`
	EnrollmentId NullableString `json:"enrollmentId,omitempty"`
	ChildrenAccountsCount *int32 `json:"childrenAccountsCount,omitempty"`
	AccountCategory *string `json:"accountCategory,omitempty"`
	IsContra *bool `json:"isContra,omitempty"`
	IsMonetary *bool `json:"isMonetary,omitempty"`
	IncomeStatementSubType NullableString `json:"incomeStatementSubType,omitempty"`
	NormalBalance *string `json:"normalBalance,omitempty"`
	BalanceAmount *Money `json:"balanceAmount,omitempty"`
	CreditsBalanceAmount *Money `json:"creditsBalanceAmount,omitempty"`
	DebitsBalanceAmount *Money `json:"debitsBalanceAmount,omitempty"`
	BalanceAmountInUsd *Money `json:"balanceAmountInUsd,omitempty"`
	DebitsBalanceAmountInUsd *Money `json:"debitsBalanceAmountInUsd,omitempty"`
	CreditsBalanceAmountInUsd *Money `json:"creditsBalanceAmountInUsd,omitempty"`
}

// NewAccountDto instantiates a new AccountDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAccountDto() *AccountDto {
	this := AccountDto{}
	return &this
}

// NewAccountDtoWithDefaults instantiates a new AccountDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAccountDtoWithDefaults() *AccountDto {
	this := AccountDto{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *AccountDto) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *AccountDto) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *AccountDto) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *AccountDto) UnsetId() {
	o.Id.Unset()
}

// GetTimestamp returns the Timestamp field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetTimestamp() time.Time {
	if o == nil || IsNil(o.Timestamp.Get()) {
		var ret time.Time
		return ret
	}
	return *o.Timestamp.Get()
}

// GetTimestampOk returns a tuple with the Timestamp field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetTimestampOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.Timestamp.Get(), o.Timestamp.IsSet()
}

// HasTimestamp returns a boolean if a field has been set.
func (o *AccountDto) HasTimestamp() bool {
	if o != nil && o.Timestamp.IsSet() {
		return true
	}

	return false
}

// SetTimestamp gets a reference to the given NullableTime and assigns it to the Timestamp field.
func (o *AccountDto) SetTimestamp(v time.Time) {
	o.Timestamp.Set(&v)
}
// SetTimestampNil sets the value for Timestamp to be an explicit nil
func (o *AccountDto) SetTimestampNil() {
	o.Timestamp.Set(nil)
}

// UnsetTimestamp ensures that no value is present for Timestamp, not even an explicit nil
func (o *AccountDto) UnsetTimestamp() {
	o.Timestamp.Unset()
}

// GetGroup returns the Group field value if set, zero value otherwise.
func (o *AccountDto) GetGroup() bool {
	if o == nil || IsNil(o.Group) {
		var ret bool
		return ret
	}
	return *o.Group
}

// GetGroupOk returns a tuple with the Group field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetGroupOk() (*bool, bool) {
	if o == nil || IsNil(o.Group) {
		return nil, false
	}
	return o.Group, true
}

// HasGroup returns a boolean if a field has been set.
func (o *AccountDto) HasGroup() bool {
	if o != nil && !IsNil(o.Group) {
		return true
	}

	return false
}

// SetGroup gets a reference to the given bool and assigns it to the Group field.
func (o *AccountDto) SetGroup(v bool) {
	o.Group = &v
}

// GetFrozen returns the Frozen field value if set, zero value otherwise.
func (o *AccountDto) GetFrozen() bool {
	if o == nil || IsNil(o.Frozen) {
		var ret bool
		return ret
	}
	return *o.Frozen
}

// GetFrozenOk returns a tuple with the Frozen field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetFrozenOk() (*bool, bool) {
	if o == nil || IsNil(o.Frozen) {
		return nil, false
	}
	return o.Frozen, true
}

// HasFrozen returns a boolean if a field has been set.
func (o *AccountDto) HasFrozen() bool {
	if o != nil && !IsNil(o.Frozen) {
		return true
	}

	return false
}

// SetFrozen gets a reference to the given bool and assigns it to the Frozen field.
func (o *AccountDto) SetFrozen(v bool) {
	o.Frozen = &v
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *AccountDto) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *AccountDto) SetName(v string) {
	o.Name.Set(&v)
}
// SetNameNil sets the value for Name to be an explicit nil
func (o *AccountDto) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *AccountDto) UnsetName() {
	o.Name.Unset()
}

// GetCode returns the Code field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetCode() string {
	if o == nil || IsNil(o.Code.Get()) {
		var ret string
		return ret
	}
	return *o.Code.Get()
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Code.Get(), o.Code.IsSet()
}

// HasCode returns a boolean if a field has been set.
func (o *AccountDto) HasCode() bool {
	if o != nil && o.Code.IsSet() {
		return true
	}

	return false
}

// SetCode gets a reference to the given NullableString and assigns it to the Code field.
func (o *AccountDto) SetCode(v string) {
	o.Code.Set(&v)
}
// SetCodeNil sets the value for Code to be an explicit nil
func (o *AccountDto) SetCodeNil() {
	o.Code.Set(nil)
}

// UnsetCode ensures that no value is present for Code, not even an explicit nil
func (o *AccountDto) UnsetCode() {
	o.Code.Unset()
}

// GetPath returns the Path field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetPath() string {
	if o == nil || IsNil(o.Path.Get()) {
		var ret string
		return ret
	}
	return *o.Path.Get()
}

// GetPathOk returns a tuple with the Path field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetPathOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Path.Get(), o.Path.IsSet()
}

// HasPath returns a boolean if a field has been set.
func (o *AccountDto) HasPath() bool {
	if o != nil && o.Path.IsSet() {
		return true
	}

	return false
}

// SetPath gets a reference to the given NullableString and assigns it to the Path field.
func (o *AccountDto) SetPath(v string) {
	o.Path.Set(&v)
}
// SetPathNil sets the value for Path to be an explicit nil
func (o *AccountDto) SetPathNil() {
	o.Path.Set(nil)
}

// UnsetPath ensures that no value is present for Path, not even an explicit nil
func (o *AccountDto) UnsetPath() {
	o.Path.Unset()
}

// GetTitle returns the Title field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetTitle() string {
	if o == nil || IsNil(o.Title.Get()) {
		var ret string
		return ret
	}
	return *o.Title.Get()
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Title.Get(), o.Title.IsSet()
}

// HasTitle returns a boolean if a field has been set.
func (o *AccountDto) HasTitle() bool {
	if o != nil && o.Title.IsSet() {
		return true
	}

	return false
}

// SetTitle gets a reference to the given NullableString and assigns it to the Title field.
func (o *AccountDto) SetTitle(v string) {
	o.Title.Set(&v)
}
// SetTitleNil sets the value for Title to be an explicit nil
func (o *AccountDto) SetTitleNil() {
	o.Title.Set(nil)
}

// UnsetTitle ensures that no value is present for Title, not even an explicit nil
func (o *AccountDto) UnsetTitle() {
	o.Title.Unset()
}

// GetPrefix returns the Prefix field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetPrefix() string {
	if o == nil || IsNil(o.Prefix.Get()) {
		var ret string
		return ret
	}
	return *o.Prefix.Get()
}

// GetPrefixOk returns a tuple with the Prefix field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetPrefixOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Prefix.Get(), o.Prefix.IsSet()
}

// HasPrefix returns a boolean if a field has been set.
func (o *AccountDto) HasPrefix() bool {
	if o != nil && o.Prefix.IsSet() {
		return true
	}

	return false
}

// SetPrefix gets a reference to the given NullableString and assigns it to the Prefix field.
func (o *AccountDto) SetPrefix(v string) {
	o.Prefix.Set(&v)
}
// SetPrefixNil sets the value for Prefix to be an explicit nil
func (o *AccountDto) SetPrefixNil() {
	o.Prefix.Set(nil)
}

// UnsetPrefix ensures that no value is present for Prefix, not even an explicit nil
func (o *AccountDto) UnsetPrefix() {
	o.Prefix.Unset()
}

// GetBalance returns the Balance field value if set, zero value otherwise.
func (o *AccountDto) GetBalance() float64 {
	if o == nil || IsNil(o.Balance) {
		var ret float64
		return ret
	}
	return *o.Balance
}

// GetBalanceOk returns a tuple with the Balance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetBalanceOk() (*float64, bool) {
	if o == nil || IsNil(o.Balance) {
		return nil, false
	}
	return o.Balance, true
}

// HasBalance returns a boolean if a field has been set.
func (o *AccountDto) HasBalance() bool {
	if o != nil && !IsNil(o.Balance) {
		return true
	}

	return false
}

// SetBalance gets a reference to the given float64 and assigns it to the Balance field.
func (o *AccountDto) SetBalance(v float64) {
	o.Balance = &v
}

// GetCurrencyId returns the CurrencyId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetCurrencyId() string {
	if o == nil || IsNil(o.CurrencyId.Get()) {
		var ret string
		return ret
	}
	return *o.CurrencyId.Get()
}

// GetCurrencyIdOk returns a tuple with the CurrencyId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetCurrencyIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CurrencyId.Get(), o.CurrencyId.IsSet()
}

// HasCurrencyId returns a boolean if a field has been set.
func (o *AccountDto) HasCurrencyId() bool {
	if o != nil && o.CurrencyId.IsSet() {
		return true
	}

	return false
}

// SetCurrencyId gets a reference to the given NullableString and assigns it to the CurrencyId field.
func (o *AccountDto) SetCurrencyId(v string) {
	o.CurrencyId.Set(&v)
}
// SetCurrencyIdNil sets the value for CurrencyId to be an explicit nil
func (o *AccountDto) SetCurrencyIdNil() {
	o.CurrencyId.Set(nil)
}

// UnsetCurrencyId ensures that no value is present for CurrencyId, not even an explicit nil
func (o *AccountDto) UnsetCurrencyId() {
	o.CurrencyId.Unset()
}

// GetContactId returns the ContactId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetContactId() string {
	if o == nil || IsNil(o.ContactId.Get()) {
		var ret string
		return ret
	}
	return *o.ContactId.Get()
}

// GetContactIdOk returns a tuple with the ContactId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetContactIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ContactId.Get(), o.ContactId.IsSet()
}

// HasContactId returns a boolean if a field has been set.
func (o *AccountDto) HasContactId() bool {
	if o != nil && o.ContactId.IsSet() {
		return true
	}

	return false
}

// SetContactId gets a reference to the given NullableString and assigns it to the ContactId field.
func (o *AccountDto) SetContactId(v string) {
	o.ContactId.Set(&v)
}
// SetContactIdNil sets the value for ContactId to be an explicit nil
func (o *AccountDto) SetContactIdNil() {
	o.ContactId.Set(nil)
}

// UnsetContactId ensures that no value is present for ContactId, not even an explicit nil
func (o *AccountDto) UnsetContactId() {
	o.ContactId.Unset()
}

// GetAccountType returns the AccountType field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetAccountType() string {
	if o == nil || IsNil(o.AccountType.Get()) {
		var ret string
		return ret
	}
	return *o.AccountType.Get()
}

// GetAccountTypeOk returns a tuple with the AccountType field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetAccountTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.AccountType.Get(), o.AccountType.IsSet()
}

// HasAccountType returns a boolean if a field has been set.
func (o *AccountDto) HasAccountType() bool {
	if o != nil && o.AccountType.IsSet() {
		return true
	}

	return false
}

// SetAccountType gets a reference to the given NullableString and assigns it to the AccountType field.
func (o *AccountDto) SetAccountType(v string) {
	o.AccountType.Set(&v)
}
// SetAccountTypeNil sets the value for AccountType to be an explicit nil
func (o *AccountDto) SetAccountTypeNil() {
	o.AccountType.Set(nil)
}

// UnsetAccountType ensures that no value is present for AccountType, not even an explicit nil
func (o *AccountDto) UnsetAccountType() {
	o.AccountType.Unset()
}

// GetQualifiedName returns the QualifiedName field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetQualifiedName() string {
	if o == nil || IsNil(o.QualifiedName.Get()) {
		var ret string
		return ret
	}
	return *o.QualifiedName.Get()
}

// GetQualifiedNameOk returns a tuple with the QualifiedName field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetQualifiedNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.QualifiedName.Get(), o.QualifiedName.IsSet()
}

// HasQualifiedName returns a boolean if a field has been set.
func (o *AccountDto) HasQualifiedName() bool {
	if o != nil && o.QualifiedName.IsSet() {
		return true
	}

	return false
}

// SetQualifiedName gets a reference to the given NullableString and assigns it to the QualifiedName field.
func (o *AccountDto) SetQualifiedName(v string) {
	o.QualifiedName.Set(&v)
}
// SetQualifiedNameNil sets the value for QualifiedName to be an explicit nil
func (o *AccountDto) SetQualifiedNameNil() {
	o.QualifiedName.Set(nil)
}

// UnsetQualifiedName ensures that no value is present for QualifiedName, not even an explicit nil
func (o *AccountDto) UnsetQualifiedName() {
	o.QualifiedName.Unset()
}

// GetAccountTypeId returns the AccountTypeId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetAccountTypeId() string {
	if o == nil || IsNil(o.AccountTypeId.Get()) {
		var ret string
		return ret
	}
	return *o.AccountTypeId.Get()
}

// GetAccountTypeIdOk returns a tuple with the AccountTypeId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetAccountTypeIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.AccountTypeId.Get(), o.AccountTypeId.IsSet()
}

// HasAccountTypeId returns a boolean if a field has been set.
func (o *AccountDto) HasAccountTypeId() bool {
	if o != nil && o.AccountTypeId.IsSet() {
		return true
	}

	return false
}

// SetAccountTypeId gets a reference to the given NullableString and assigns it to the AccountTypeId field.
func (o *AccountDto) SetAccountTypeId(v string) {
	o.AccountTypeId.Set(&v)
}
// SetAccountTypeIdNil sets the value for AccountTypeId to be an explicit nil
func (o *AccountDto) SetAccountTypeIdNil() {
	o.AccountTypeId.Set(nil)
}

// UnsetAccountTypeId ensures that no value is present for AccountTypeId, not even an explicit nil
func (o *AccountDto) UnsetAccountTypeId() {
	o.AccountTypeId.Unset()
}

// GetDebitsBalance returns the DebitsBalance field value if set, zero value otherwise.
func (o *AccountDto) GetDebitsBalance() float64 {
	if o == nil || IsNil(o.DebitsBalance) {
		var ret float64
		return ret
	}
	return *o.DebitsBalance
}

// GetDebitsBalanceOk returns a tuple with the DebitsBalance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetDebitsBalanceOk() (*float64, bool) {
	if o == nil || IsNil(o.DebitsBalance) {
		return nil, false
	}
	return o.DebitsBalance, true
}

// HasDebitsBalance returns a boolean if a field has been set.
func (o *AccountDto) HasDebitsBalance() bool {
	if o != nil && !IsNil(o.DebitsBalance) {
		return true
	}

	return false
}

// SetDebitsBalance gets a reference to the given float64 and assigns it to the DebitsBalance field.
func (o *AccountDto) SetDebitsBalance(v float64) {
	o.DebitsBalance = &v
}

// GetCreditsBalance returns the CreditsBalance field value if set, zero value otherwise.
func (o *AccountDto) GetCreditsBalance() float64 {
	if o == nil || IsNil(o.CreditsBalance) {
		var ret float64
		return ret
	}
	return *o.CreditsBalance
}

// GetCreditsBalanceOk returns a tuple with the CreditsBalance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetCreditsBalanceOk() (*float64, bool) {
	if o == nil || IsNil(o.CreditsBalance) {
		return nil, false
	}
	return o.CreditsBalance, true
}

// HasCreditsBalance returns a boolean if a field has been set.
func (o *AccountDto) HasCreditsBalance() bool {
	if o != nil && !IsNil(o.CreditsBalance) {
		return true
	}

	return false
}

// SetCreditsBalance gets a reference to the given float64 and assigns it to the CreditsBalance field.
func (o *AccountDto) SetCreditsBalance(v float64) {
	o.CreditsBalance = &v
}

// GetBalanceInUsd returns the BalanceInUsd field value if set, zero value otherwise.
func (o *AccountDto) GetBalanceInUsd() float64 {
	if o == nil || IsNil(o.BalanceInUsd) {
		var ret float64
		return ret
	}
	return *o.BalanceInUsd
}

// GetBalanceInUsdOk returns a tuple with the BalanceInUsd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetBalanceInUsdOk() (*float64, bool) {
	if o == nil || IsNil(o.BalanceInUsd) {
		return nil, false
	}
	return o.BalanceInUsd, true
}

// HasBalanceInUsd returns a boolean if a field has been set.
func (o *AccountDto) HasBalanceInUsd() bool {
	if o != nil && !IsNil(o.BalanceInUsd) {
		return true
	}

	return false
}

// SetBalanceInUsd gets a reference to the given float64 and assigns it to the BalanceInUsd field.
func (o *AccountDto) SetBalanceInUsd(v float64) {
	o.BalanceInUsd = &v
}

// GetDebitsBalanceInUsd returns the DebitsBalanceInUsd field value if set, zero value otherwise.
func (o *AccountDto) GetDebitsBalanceInUsd() float64 {
	if o == nil || IsNil(o.DebitsBalanceInUsd) {
		var ret float64
		return ret
	}
	return *o.DebitsBalanceInUsd
}

// GetDebitsBalanceInUsdOk returns a tuple with the DebitsBalanceInUsd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetDebitsBalanceInUsdOk() (*float64, bool) {
	if o == nil || IsNil(o.DebitsBalanceInUsd) {
		return nil, false
	}
	return o.DebitsBalanceInUsd, true
}

// HasDebitsBalanceInUsd returns a boolean if a field has been set.
func (o *AccountDto) HasDebitsBalanceInUsd() bool {
	if o != nil && !IsNil(o.DebitsBalanceInUsd) {
		return true
	}

	return false
}

// SetDebitsBalanceInUsd gets a reference to the given float64 and assigns it to the DebitsBalanceInUsd field.
func (o *AccountDto) SetDebitsBalanceInUsd(v float64) {
	o.DebitsBalanceInUsd = &v
}

// GetCreditsBalanceInUsd returns the CreditsBalanceInUsd field value if set, zero value otherwise.
func (o *AccountDto) GetCreditsBalanceInUsd() float64 {
	if o == nil || IsNil(o.CreditsBalanceInUsd) {
		var ret float64
		return ret
	}
	return *o.CreditsBalanceInUsd
}

// GetCreditsBalanceInUsdOk returns a tuple with the CreditsBalanceInUsd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetCreditsBalanceInUsdOk() (*float64, bool) {
	if o == nil || IsNil(o.CreditsBalanceInUsd) {
		return nil, false
	}
	return o.CreditsBalanceInUsd, true
}

// HasCreditsBalanceInUsd returns a boolean if a field has been set.
func (o *AccountDto) HasCreditsBalanceInUsd() bool {
	if o != nil && !IsNil(o.CreditsBalanceInUsd) {
		return true
	}

	return false
}

// SetCreditsBalanceInUsd gets a reference to the given float64 and assigns it to the CreditsBalanceInUsd field.
func (o *AccountDto) SetCreditsBalanceInUsd(v float64) {
	o.CreditsBalanceInUsd = &v
}

// GetForexRate returns the ForexRate field value if set, zero value otherwise.
func (o *AccountDto) GetForexRate() float64 {
	if o == nil || IsNil(o.ForexRate) {
		var ret float64
		return ret
	}
	return *o.ForexRate
}

// GetForexRateOk returns a tuple with the ForexRate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetForexRateOk() (*float64, bool) {
	if o == nil || IsNil(o.ForexRate) {
		return nil, false
	}
	return o.ForexRate, true
}

// HasForexRate returns a boolean if a field has been set.
func (o *AccountDto) HasForexRate() bool {
	if o != nil && !IsNil(o.ForexRate) {
		return true
	}

	return false
}

// SetForexRate gets a reference to the given float64 and assigns it to the ForexRate field.
func (o *AccountDto) SetForexRate(v float64) {
	o.ForexRate = &v
}

// GetParentAccountId returns the ParentAccountId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetParentAccountId() string {
	if o == nil || IsNil(o.ParentAccountId.Get()) {
		var ret string
		return ret
	}
	return *o.ParentAccountId.Get()
}

// GetParentAccountIdOk returns a tuple with the ParentAccountId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetParentAccountIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ParentAccountId.Get(), o.ParentAccountId.IsSet()
}

// HasParentAccountId returns a boolean if a field has been set.
func (o *AccountDto) HasParentAccountId() bool {
	if o != nil && o.ParentAccountId.IsSet() {
		return true
	}

	return false
}

// SetParentAccountId gets a reference to the given NullableString and assigns it to the ParentAccountId field.
func (o *AccountDto) SetParentAccountId(v string) {
	o.ParentAccountId.Set(&v)
}
// SetParentAccountIdNil sets the value for ParentAccountId to be an explicit nil
func (o *AccountDto) SetParentAccountIdNil() {
	o.ParentAccountId.Set(nil)
}

// UnsetParentAccountId ensures that no value is present for ParentAccountId, not even an explicit nil
func (o *AccountDto) UnsetParentAccountId() {
	o.ParentAccountId.Unset()
}

// GetTenantId returns the TenantId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetTenantId() string {
	if o == nil || IsNil(o.TenantId.Get()) {
		var ret string
		return ret
	}
	return *o.TenantId.Get()
}

// GetTenantIdOk returns a tuple with the TenantId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetTenantIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.TenantId.Get(), o.TenantId.IsSet()
}

// HasTenantId returns a boolean if a field has been set.
func (o *AccountDto) HasTenantId() bool {
	if o != nil && o.TenantId.IsSet() {
		return true
	}

	return false
}

// SetTenantId gets a reference to the given NullableString and assigns it to the TenantId field.
func (o *AccountDto) SetTenantId(v string) {
	o.TenantId.Set(&v)
}
// SetTenantIdNil sets the value for TenantId to be an explicit nil
func (o *AccountDto) SetTenantIdNil() {
	o.TenantId.Set(nil)
}

// UnsetTenantId ensures that no value is present for TenantId, not even an explicit nil
func (o *AccountDto) UnsetTenantId() {
	o.TenantId.Unset()
}

// GetEnrollmentId returns the EnrollmentId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetEnrollmentId() string {
	if o == nil || IsNil(o.EnrollmentId.Get()) {
		var ret string
		return ret
	}
	return *o.EnrollmentId.Get()
}

// GetEnrollmentIdOk returns a tuple with the EnrollmentId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetEnrollmentIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.EnrollmentId.Get(), o.EnrollmentId.IsSet()
}

// HasEnrollmentId returns a boolean if a field has been set.
func (o *AccountDto) HasEnrollmentId() bool {
	if o != nil && o.EnrollmentId.IsSet() {
		return true
	}

	return false
}

// SetEnrollmentId gets a reference to the given NullableString and assigns it to the EnrollmentId field.
func (o *AccountDto) SetEnrollmentId(v string) {
	o.EnrollmentId.Set(&v)
}
// SetEnrollmentIdNil sets the value for EnrollmentId to be an explicit nil
func (o *AccountDto) SetEnrollmentIdNil() {
	o.EnrollmentId.Set(nil)
}

// UnsetEnrollmentId ensures that no value is present for EnrollmentId, not even an explicit nil
func (o *AccountDto) UnsetEnrollmentId() {
	o.EnrollmentId.Unset()
}

// GetChildrenAccountsCount returns the ChildrenAccountsCount field value if set, zero value otherwise.
func (o *AccountDto) GetChildrenAccountsCount() int32 {
	if o == nil || IsNil(o.ChildrenAccountsCount) {
		var ret int32
		return ret
	}
	return *o.ChildrenAccountsCount
}

// GetChildrenAccountsCountOk returns a tuple with the ChildrenAccountsCount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetChildrenAccountsCountOk() (*int32, bool) {
	if o == nil || IsNil(o.ChildrenAccountsCount) {
		return nil, false
	}
	return o.ChildrenAccountsCount, true
}

// HasChildrenAccountsCount returns a boolean if a field has been set.
func (o *AccountDto) HasChildrenAccountsCount() bool {
	if o != nil && !IsNil(o.ChildrenAccountsCount) {
		return true
	}

	return false
}

// SetChildrenAccountsCount gets a reference to the given int32 and assigns it to the ChildrenAccountsCount field.
func (o *AccountDto) SetChildrenAccountsCount(v int32) {
	o.ChildrenAccountsCount = &v
}

// GetAccountCategory returns the AccountCategory field value if set, zero value otherwise.
func (o *AccountDto) GetAccountCategory() string {
	if o == nil || IsNil(o.AccountCategory) {
		var ret string
		return ret
	}
	return *o.AccountCategory
}

// GetAccountCategoryOk returns a tuple with the AccountCategory field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetAccountCategoryOk() (*string, bool) {
	if o == nil || IsNil(o.AccountCategory) {
		return nil, false
	}
	return o.AccountCategory, true
}

// HasAccountCategory returns a boolean if a field has been set.
func (o *AccountDto) HasAccountCategory() bool {
	if o != nil && !IsNil(o.AccountCategory) {
		return true
	}

	return false
}

// SetAccountCategory gets a reference to the given string and assigns it to the AccountCategory field.
func (o *AccountDto) SetAccountCategory(v string) {
	o.AccountCategory = &v
}

// GetIsContra returns the IsContra field value if set, zero value otherwise.
func (o *AccountDto) GetIsContra() bool {
	if o == nil || IsNil(o.IsContra) {
		var ret bool
		return ret
	}
	return *o.IsContra
}

// GetIsContraOk returns a tuple with the IsContra field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetIsContraOk() (*bool, bool) {
	if o == nil || IsNil(o.IsContra) {
		return nil, false
	}
	return o.IsContra, true
}

// HasIsContra returns a boolean if a field has been set.
func (o *AccountDto) HasIsContra() bool {
	if o != nil && !IsNil(o.IsContra) {
		return true
	}

	return false
}

// SetIsContra gets a reference to the given bool and assigns it to the IsContra field.
func (o *AccountDto) SetIsContra(v bool) {
	o.IsContra = &v
}

// GetIsMonetary returns the IsMonetary field value if set, zero value otherwise.
func (o *AccountDto) GetIsMonetary() bool {
	if o == nil || IsNil(o.IsMonetary) {
		var ret bool
		return ret
	}
	return *o.IsMonetary
}

// GetIsMonetaryOk returns a tuple with the IsMonetary field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetIsMonetaryOk() (*bool, bool) {
	if o == nil || IsNil(o.IsMonetary) {
		return nil, false
	}
	return o.IsMonetary, true
}

// HasIsMonetary returns a boolean if a field has been set.
func (o *AccountDto) HasIsMonetary() bool {
	if o != nil && !IsNil(o.IsMonetary) {
		return true
	}

	return false
}

// SetIsMonetary gets a reference to the given bool and assigns it to the IsMonetary field.
func (o *AccountDto) SetIsMonetary(v bool) {
	o.IsMonetary = &v
}

// GetIncomeStatementSubType returns the IncomeStatementSubType field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *AccountDto) GetIncomeStatementSubType() string {
	if o == nil || IsNil(o.IncomeStatementSubType.Get()) {
		var ret string
		return ret
	}
	return *o.IncomeStatementSubType.Get()
}

// GetIncomeStatementSubTypeOk returns a tuple with the IncomeStatementSubType field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *AccountDto) GetIncomeStatementSubTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.IncomeStatementSubType.Get(), o.IncomeStatementSubType.IsSet()
}

// HasIncomeStatementSubType returns a boolean if a field has been set.
func (o *AccountDto) HasIncomeStatementSubType() bool {
	if o != nil && o.IncomeStatementSubType.IsSet() {
		return true
	}

	return false
}

// SetIncomeStatementSubType gets a reference to the given NullableString and assigns it to the IncomeStatementSubType field.
func (o *AccountDto) SetIncomeStatementSubType(v string) {
	o.IncomeStatementSubType.Set(&v)
}
// SetIncomeStatementSubTypeNil sets the value for IncomeStatementSubType to be an explicit nil
func (o *AccountDto) SetIncomeStatementSubTypeNil() {
	o.IncomeStatementSubType.Set(nil)
}

// UnsetIncomeStatementSubType ensures that no value is present for IncomeStatementSubType, not even an explicit nil
func (o *AccountDto) UnsetIncomeStatementSubType() {
	o.IncomeStatementSubType.Unset()
}

// GetNormalBalance returns the NormalBalance field value if set, zero value otherwise.
func (o *AccountDto) GetNormalBalance() string {
	if o == nil || IsNil(o.NormalBalance) {
		var ret string
		return ret
	}
	return *o.NormalBalance
}

// GetNormalBalanceOk returns a tuple with the NormalBalance field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetNormalBalanceOk() (*string, bool) {
	if o == nil || IsNil(o.NormalBalance) {
		return nil, false
	}
	return o.NormalBalance, true
}

// HasNormalBalance returns a boolean if a field has been set.
func (o *AccountDto) HasNormalBalance() bool {
	if o != nil && !IsNil(o.NormalBalance) {
		return true
	}

	return false
}

// SetNormalBalance gets a reference to the given string and assigns it to the NormalBalance field.
func (o *AccountDto) SetNormalBalance(v string) {
	o.NormalBalance = &v
}

// GetBalanceAmount returns the BalanceAmount field value if set, zero value otherwise.
func (o *AccountDto) GetBalanceAmount() Money {
	if o == nil || IsNil(o.BalanceAmount) {
		var ret Money
		return ret
	}
	return *o.BalanceAmount
}

// GetBalanceAmountOk returns a tuple with the BalanceAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetBalanceAmountOk() (*Money, bool) {
	if o == nil || IsNil(o.BalanceAmount) {
		return nil, false
	}
	return o.BalanceAmount, true
}

// HasBalanceAmount returns a boolean if a field has been set.
func (o *AccountDto) HasBalanceAmount() bool {
	if o != nil && !IsNil(o.BalanceAmount) {
		return true
	}

	return false
}

// SetBalanceAmount gets a reference to the given Money and assigns it to the BalanceAmount field.
func (o *AccountDto) SetBalanceAmount(v Money) {
	o.BalanceAmount = &v
}

// GetCreditsBalanceAmount returns the CreditsBalanceAmount field value if set, zero value otherwise.
func (o *AccountDto) GetCreditsBalanceAmount() Money {
	if o == nil || IsNil(o.CreditsBalanceAmount) {
		var ret Money
		return ret
	}
	return *o.CreditsBalanceAmount
}

// GetCreditsBalanceAmountOk returns a tuple with the CreditsBalanceAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetCreditsBalanceAmountOk() (*Money, bool) {
	if o == nil || IsNil(o.CreditsBalanceAmount) {
		return nil, false
	}
	return o.CreditsBalanceAmount, true
}

// HasCreditsBalanceAmount returns a boolean if a field has been set.
func (o *AccountDto) HasCreditsBalanceAmount() bool {
	if o != nil && !IsNil(o.CreditsBalanceAmount) {
		return true
	}

	return false
}

// SetCreditsBalanceAmount gets a reference to the given Money and assigns it to the CreditsBalanceAmount field.
func (o *AccountDto) SetCreditsBalanceAmount(v Money) {
	o.CreditsBalanceAmount = &v
}

// GetDebitsBalanceAmount returns the DebitsBalanceAmount field value if set, zero value otherwise.
func (o *AccountDto) GetDebitsBalanceAmount() Money {
	if o == nil || IsNil(o.DebitsBalanceAmount) {
		var ret Money
		return ret
	}
	return *o.DebitsBalanceAmount
}

// GetDebitsBalanceAmountOk returns a tuple with the DebitsBalanceAmount field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetDebitsBalanceAmountOk() (*Money, bool) {
	if o == nil || IsNil(o.DebitsBalanceAmount) {
		return nil, false
	}
	return o.DebitsBalanceAmount, true
}

// HasDebitsBalanceAmount returns a boolean if a field has been set.
func (o *AccountDto) HasDebitsBalanceAmount() bool {
	if o != nil && !IsNil(o.DebitsBalanceAmount) {
		return true
	}

	return false
}

// SetDebitsBalanceAmount gets a reference to the given Money and assigns it to the DebitsBalanceAmount field.
func (o *AccountDto) SetDebitsBalanceAmount(v Money) {
	o.DebitsBalanceAmount = &v
}

// GetBalanceAmountInUsd returns the BalanceAmountInUsd field value if set, zero value otherwise.
func (o *AccountDto) GetBalanceAmountInUsd() Money {
	if o == nil || IsNil(o.BalanceAmountInUsd) {
		var ret Money
		return ret
	}
	return *o.BalanceAmountInUsd
}

// GetBalanceAmountInUsdOk returns a tuple with the BalanceAmountInUsd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetBalanceAmountInUsdOk() (*Money, bool) {
	if o == nil || IsNil(o.BalanceAmountInUsd) {
		return nil, false
	}
	return o.BalanceAmountInUsd, true
}

// HasBalanceAmountInUsd returns a boolean if a field has been set.
func (o *AccountDto) HasBalanceAmountInUsd() bool {
	if o != nil && !IsNil(o.BalanceAmountInUsd) {
		return true
	}

	return false
}

// SetBalanceAmountInUsd gets a reference to the given Money and assigns it to the BalanceAmountInUsd field.
func (o *AccountDto) SetBalanceAmountInUsd(v Money) {
	o.BalanceAmountInUsd = &v
}

// GetDebitsBalanceAmountInUsd returns the DebitsBalanceAmountInUsd field value if set, zero value otherwise.
func (o *AccountDto) GetDebitsBalanceAmountInUsd() Money {
	if o == nil || IsNil(o.DebitsBalanceAmountInUsd) {
		var ret Money
		return ret
	}
	return *o.DebitsBalanceAmountInUsd
}

// GetDebitsBalanceAmountInUsdOk returns a tuple with the DebitsBalanceAmountInUsd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetDebitsBalanceAmountInUsdOk() (*Money, bool) {
	if o == nil || IsNil(o.DebitsBalanceAmountInUsd) {
		return nil, false
	}
	return o.DebitsBalanceAmountInUsd, true
}

// HasDebitsBalanceAmountInUsd returns a boolean if a field has been set.
func (o *AccountDto) HasDebitsBalanceAmountInUsd() bool {
	if o != nil && !IsNil(o.DebitsBalanceAmountInUsd) {
		return true
	}

	return false
}

// SetDebitsBalanceAmountInUsd gets a reference to the given Money and assigns it to the DebitsBalanceAmountInUsd field.
func (o *AccountDto) SetDebitsBalanceAmountInUsd(v Money) {
	o.DebitsBalanceAmountInUsd = &v
}

// GetCreditsBalanceAmountInUsd returns the CreditsBalanceAmountInUsd field value if set, zero value otherwise.
func (o *AccountDto) GetCreditsBalanceAmountInUsd() Money {
	if o == nil || IsNil(o.CreditsBalanceAmountInUsd) {
		var ret Money
		return ret
	}
	return *o.CreditsBalanceAmountInUsd
}

// GetCreditsBalanceAmountInUsdOk returns a tuple with the CreditsBalanceAmountInUsd field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AccountDto) GetCreditsBalanceAmountInUsdOk() (*Money, bool) {
	if o == nil || IsNil(o.CreditsBalanceAmountInUsd) {
		return nil, false
	}
	return o.CreditsBalanceAmountInUsd, true
}

// HasCreditsBalanceAmountInUsd returns a boolean if a field has been set.
func (o *AccountDto) HasCreditsBalanceAmountInUsd() bool {
	if o != nil && !IsNil(o.CreditsBalanceAmountInUsd) {
		return true
	}

	return false
}

// SetCreditsBalanceAmountInUsd gets a reference to the given Money and assigns it to the CreditsBalanceAmountInUsd field.
func (o *AccountDto) SetCreditsBalanceAmountInUsd(v Money) {
	o.CreditsBalanceAmountInUsd = &v
}

func (o AccountDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AccountDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if o.Timestamp.IsSet() {
		toSerialize["timestamp"] = o.Timestamp.Get()
	}
	if !IsNil(o.Group) {
		toSerialize["group"] = o.Group
	}
	if !IsNil(o.Frozen) {
		toSerialize["frozen"] = o.Frozen
	}
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	if o.Code.IsSet() {
		toSerialize["code"] = o.Code.Get()
	}
	if o.Path.IsSet() {
		toSerialize["path"] = o.Path.Get()
	}
	if o.Title.IsSet() {
		toSerialize["title"] = o.Title.Get()
	}
	if o.Prefix.IsSet() {
		toSerialize["prefix"] = o.Prefix.Get()
	}
	if !IsNil(o.Balance) {
		toSerialize["balance"] = o.Balance
	}
	if o.CurrencyId.IsSet() {
		toSerialize["currencyId"] = o.CurrencyId.Get()
	}
	if o.ContactId.IsSet() {
		toSerialize["contactId"] = o.ContactId.Get()
	}
	if o.AccountType.IsSet() {
		toSerialize["accountType"] = o.AccountType.Get()
	}
	if o.QualifiedName.IsSet() {
		toSerialize["qualifiedName"] = o.QualifiedName.Get()
	}
	if o.AccountTypeId.IsSet() {
		toSerialize["accountTypeId"] = o.AccountTypeId.Get()
	}
	if !IsNil(o.DebitsBalance) {
		toSerialize["debitsBalance"] = o.DebitsBalance
	}
	if !IsNil(o.CreditsBalance) {
		toSerialize["creditsBalance"] = o.CreditsBalance
	}
	if !IsNil(o.BalanceInUsd) {
		toSerialize["balanceInUsd"] = o.BalanceInUsd
	}
	if !IsNil(o.DebitsBalanceInUsd) {
		toSerialize["debitsBalanceInUsd"] = o.DebitsBalanceInUsd
	}
	if !IsNil(o.CreditsBalanceInUsd) {
		toSerialize["creditsBalanceInUsd"] = o.CreditsBalanceInUsd
	}
	if !IsNil(o.ForexRate) {
		toSerialize["forexRate"] = o.ForexRate
	}
	if o.ParentAccountId.IsSet() {
		toSerialize["parentAccountId"] = o.ParentAccountId.Get()
	}
	if o.TenantId.IsSet() {
		toSerialize["tenantId"] = o.TenantId.Get()
	}
	if o.EnrollmentId.IsSet() {
		toSerialize["enrollmentId"] = o.EnrollmentId.Get()
	}
	if !IsNil(o.ChildrenAccountsCount) {
		toSerialize["childrenAccountsCount"] = o.ChildrenAccountsCount
	}
	if !IsNil(o.AccountCategory) {
		toSerialize["accountCategory"] = o.AccountCategory
	}
	if !IsNil(o.IsContra) {
		toSerialize["isContra"] = o.IsContra
	}
	if !IsNil(o.IsMonetary) {
		toSerialize["isMonetary"] = o.IsMonetary
	}
	if o.IncomeStatementSubType.IsSet() {
		toSerialize["incomeStatementSubType"] = o.IncomeStatementSubType.Get()
	}
	if !IsNil(o.NormalBalance) {
		toSerialize["normalBalance"] = o.NormalBalance
	}
	if !IsNil(o.BalanceAmount) {
		toSerialize["balanceAmount"] = o.BalanceAmount
	}
	if !IsNil(o.CreditsBalanceAmount) {
		toSerialize["creditsBalanceAmount"] = o.CreditsBalanceAmount
	}
	if !IsNil(o.DebitsBalanceAmount) {
		toSerialize["debitsBalanceAmount"] = o.DebitsBalanceAmount
	}
	if !IsNil(o.BalanceAmountInUsd) {
		toSerialize["balanceAmountInUsd"] = o.BalanceAmountInUsd
	}
	if !IsNil(o.DebitsBalanceAmountInUsd) {
		toSerialize["debitsBalanceAmountInUsd"] = o.DebitsBalanceAmountInUsd
	}
	if !IsNil(o.CreditsBalanceAmountInUsd) {
		toSerialize["creditsBalanceAmountInUsd"] = o.CreditsBalanceAmountInUsd
	}
	return toSerialize, nil
}

type NullableAccountDto struct {
	value *AccountDto
	isSet bool
}

func (v NullableAccountDto) Get() *AccountDto {
	return v.value
}

func (v *NullableAccountDto) Set(val *AccountDto) {
	v.value = val
	v.isSet = true
}

func (v NullableAccountDto) IsSet() bool {
	return v.isSet
}

func (v *NullableAccountDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAccountDto(val *AccountDto) *NullableAccountDto {
	return &NullableAccountDto{value: val, isSet: true}
}

func (v NullableAccountDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAccountDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



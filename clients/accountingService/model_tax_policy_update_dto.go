/*
AccountingService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0.0
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the TaxPolicyUpdateDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TaxPolicyUpdateDto{}

// TaxPolicyUpdateDto struct for TaxPolicyUpdateDto
type TaxPolicyUpdateDto struct {
	Code NullableString `json:"code,omitempty"`
	Title NullableString `json:"title,omitempty"`
	Description NullableString `json:"description,omitempty"`
	IsFree *bool `json:"isFree,omitempty"`
	Reduce *bool `json:"reduce,omitempty"`
	IsEnabled *bool `json:"isEnabled,omitempty"`
	IsDefault *bool `json:"isDefault,omitempty"`
	AllowInternational *bool `json:"allowInternational,omitempty"`
	Hours *int32 `json:"hours,omitempty"`
	Days *int32 `json:"days,omitempty"`
	Weeks *int32 `json:"weeks,omitempty"`
	Months *int32 `json:"months,omitempty"`
	Years *int32 `json:"years,omitempty"`
	Value *float64 `json:"value,omitempty"`
	Percentage *float64 `json:"percentage,omitempty"`
	CurrencyId NullableString `json:"currencyId,omitempty"`
	CountryId NullableString `json:"countryId,omitempty"`
	CountryStateId NullableString `json:"countryStateId,omitempty"`
	CustomState NullableString `json:"customState,omitempty"`
	CustomCity NullableString `json:"customCity,omitempty"`
	CityId NullableString `json:"cityId,omitempty"`
	Zero *bool `json:"zero,omitempty"`
	Reduced *bool `json:"reduced,omitempty"`
	Withholding *bool `json:"withholding,omitempty"`
	TaxSchemeCode NullableString `json:"taxSchemeCode,omitempty"`
	TaxCategoryCode NullableString `json:"taxCategoryCode,omitempty"`
	FiscalAuthorityId NullableString `json:"fiscalAuthorityId,omitempty"`
}

// NewTaxPolicyUpdateDto instantiates a new TaxPolicyUpdateDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTaxPolicyUpdateDto() *TaxPolicyUpdateDto {
	this := TaxPolicyUpdateDto{}
	return &this
}

// NewTaxPolicyUpdateDtoWithDefaults instantiates a new TaxPolicyUpdateDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTaxPolicyUpdateDtoWithDefaults() *TaxPolicyUpdateDto {
	this := TaxPolicyUpdateDto{}
	return &this
}

// GetCode returns the Code field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCode() string {
	if o == nil || IsNil(o.Code.Get()) {
		var ret string
		return ret
	}
	return *o.Code.Get()
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Code.Get(), o.Code.IsSet()
}

// HasCode returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCode() bool {
	if o != nil && o.Code.IsSet() {
		return true
	}

	return false
}

// SetCode gets a reference to the given NullableString and assigns it to the Code field.
func (o *TaxPolicyUpdateDto) SetCode(v string) {
	o.Code.Set(&v)
}
// SetCodeNil sets the value for Code to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCodeNil() {
	o.Code.Set(nil)
}

// UnsetCode ensures that no value is present for Code, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCode() {
	o.Code.Unset()
}

// GetTitle returns the Title field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetTitle() string {
	if o == nil || IsNil(o.Title.Get()) {
		var ret string
		return ret
	}
	return *o.Title.Get()
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Title.Get(), o.Title.IsSet()
}

// HasTitle returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasTitle() bool {
	if o != nil && o.Title.IsSet() {
		return true
	}

	return false
}

// SetTitle gets a reference to the given NullableString and assigns it to the Title field.
func (o *TaxPolicyUpdateDto) SetTitle(v string) {
	o.Title.Set(&v)
}
// SetTitleNil sets the value for Title to be an explicit nil
func (o *TaxPolicyUpdateDto) SetTitleNil() {
	o.Title.Set(nil)
}

// UnsetTitle ensures that no value is present for Title, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetTitle() {
	o.Title.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *TaxPolicyUpdateDto) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *TaxPolicyUpdateDto) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetDescription() {
	o.Description.Unset()
}

// GetIsFree returns the IsFree field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetIsFree() bool {
	if o == nil || IsNil(o.IsFree) {
		var ret bool
		return ret
	}
	return *o.IsFree
}

// GetIsFreeOk returns a tuple with the IsFree field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetIsFreeOk() (*bool, bool) {
	if o == nil || IsNil(o.IsFree) {
		return nil, false
	}
	return o.IsFree, true
}

// HasIsFree returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasIsFree() bool {
	if o != nil && !IsNil(o.IsFree) {
		return true
	}

	return false
}

// SetIsFree gets a reference to the given bool and assigns it to the IsFree field.
func (o *TaxPolicyUpdateDto) SetIsFree(v bool) {
	o.IsFree = &v
}

// GetReduce returns the Reduce field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetReduce() bool {
	if o == nil || IsNil(o.Reduce) {
		var ret bool
		return ret
	}
	return *o.Reduce
}

// GetReduceOk returns a tuple with the Reduce field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetReduceOk() (*bool, bool) {
	if o == nil || IsNil(o.Reduce) {
		return nil, false
	}
	return o.Reduce, true
}

// HasReduce returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasReduce() bool {
	if o != nil && !IsNil(o.Reduce) {
		return true
	}

	return false
}

// SetReduce gets a reference to the given bool and assigns it to the Reduce field.
func (o *TaxPolicyUpdateDto) SetReduce(v bool) {
	o.Reduce = &v
}

// GetIsEnabled returns the IsEnabled field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetIsEnabled() bool {
	if o == nil || IsNil(o.IsEnabled) {
		var ret bool
		return ret
	}
	return *o.IsEnabled
}

// GetIsEnabledOk returns a tuple with the IsEnabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetIsEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.IsEnabled) {
		return nil, false
	}
	return o.IsEnabled, true
}

// HasIsEnabled returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasIsEnabled() bool {
	if o != nil && !IsNil(o.IsEnabled) {
		return true
	}

	return false
}

// SetIsEnabled gets a reference to the given bool and assigns it to the IsEnabled field.
func (o *TaxPolicyUpdateDto) SetIsEnabled(v bool) {
	o.IsEnabled = &v
}

// GetIsDefault returns the IsDefault field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetIsDefault() bool {
	if o == nil || IsNil(o.IsDefault) {
		var ret bool
		return ret
	}
	return *o.IsDefault
}

// GetIsDefaultOk returns a tuple with the IsDefault field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetIsDefaultOk() (*bool, bool) {
	if o == nil || IsNil(o.IsDefault) {
		return nil, false
	}
	return o.IsDefault, true
}

// HasIsDefault returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasIsDefault() bool {
	if o != nil && !IsNil(o.IsDefault) {
		return true
	}

	return false
}

// SetIsDefault gets a reference to the given bool and assigns it to the IsDefault field.
func (o *TaxPolicyUpdateDto) SetIsDefault(v bool) {
	o.IsDefault = &v
}

// GetAllowInternational returns the AllowInternational field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetAllowInternational() bool {
	if o == nil || IsNil(o.AllowInternational) {
		var ret bool
		return ret
	}
	return *o.AllowInternational
}

// GetAllowInternationalOk returns a tuple with the AllowInternational field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetAllowInternationalOk() (*bool, bool) {
	if o == nil || IsNil(o.AllowInternational) {
		return nil, false
	}
	return o.AllowInternational, true
}

// HasAllowInternational returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasAllowInternational() bool {
	if o != nil && !IsNil(o.AllowInternational) {
		return true
	}

	return false
}

// SetAllowInternational gets a reference to the given bool and assigns it to the AllowInternational field.
func (o *TaxPolicyUpdateDto) SetAllowInternational(v bool) {
	o.AllowInternational = &v
}

// GetHours returns the Hours field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetHours() int32 {
	if o == nil || IsNil(o.Hours) {
		var ret int32
		return ret
	}
	return *o.Hours
}

// GetHoursOk returns a tuple with the Hours field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetHoursOk() (*int32, bool) {
	if o == nil || IsNil(o.Hours) {
		return nil, false
	}
	return o.Hours, true
}

// HasHours returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasHours() bool {
	if o != nil && !IsNil(o.Hours) {
		return true
	}

	return false
}

// SetHours gets a reference to the given int32 and assigns it to the Hours field.
func (o *TaxPolicyUpdateDto) SetHours(v int32) {
	o.Hours = &v
}

// GetDays returns the Days field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetDays() int32 {
	if o == nil || IsNil(o.Days) {
		var ret int32
		return ret
	}
	return *o.Days
}

// GetDaysOk returns a tuple with the Days field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetDaysOk() (*int32, bool) {
	if o == nil || IsNil(o.Days) {
		return nil, false
	}
	return o.Days, true
}

// HasDays returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasDays() bool {
	if o != nil && !IsNil(o.Days) {
		return true
	}

	return false
}

// SetDays gets a reference to the given int32 and assigns it to the Days field.
func (o *TaxPolicyUpdateDto) SetDays(v int32) {
	o.Days = &v
}

// GetWeeks returns the Weeks field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetWeeks() int32 {
	if o == nil || IsNil(o.Weeks) {
		var ret int32
		return ret
	}
	return *o.Weeks
}

// GetWeeksOk returns a tuple with the Weeks field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetWeeksOk() (*int32, bool) {
	if o == nil || IsNil(o.Weeks) {
		return nil, false
	}
	return o.Weeks, true
}

// HasWeeks returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasWeeks() bool {
	if o != nil && !IsNil(o.Weeks) {
		return true
	}

	return false
}

// SetWeeks gets a reference to the given int32 and assigns it to the Weeks field.
func (o *TaxPolicyUpdateDto) SetWeeks(v int32) {
	o.Weeks = &v
}

// GetMonths returns the Months field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetMonths() int32 {
	if o == nil || IsNil(o.Months) {
		var ret int32
		return ret
	}
	return *o.Months
}

// GetMonthsOk returns a tuple with the Months field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetMonthsOk() (*int32, bool) {
	if o == nil || IsNil(o.Months) {
		return nil, false
	}
	return o.Months, true
}

// HasMonths returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasMonths() bool {
	if o != nil && !IsNil(o.Months) {
		return true
	}

	return false
}

// SetMonths gets a reference to the given int32 and assigns it to the Months field.
func (o *TaxPolicyUpdateDto) SetMonths(v int32) {
	o.Months = &v
}

// GetYears returns the Years field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetYears() int32 {
	if o == nil || IsNil(o.Years) {
		var ret int32
		return ret
	}
	return *o.Years
}

// GetYearsOk returns a tuple with the Years field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetYearsOk() (*int32, bool) {
	if o == nil || IsNil(o.Years) {
		return nil, false
	}
	return o.Years, true
}

// HasYears returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasYears() bool {
	if o != nil && !IsNil(o.Years) {
		return true
	}

	return false
}

// SetYears gets a reference to the given int32 and assigns it to the Years field.
func (o *TaxPolicyUpdateDto) SetYears(v int32) {
	o.Years = &v
}

// GetValue returns the Value field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetValue() float64 {
	if o == nil || IsNil(o.Value) {
		var ret float64
		return ret
	}
	return *o.Value
}

// GetValueOk returns a tuple with the Value field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetValueOk() (*float64, bool) {
	if o == nil || IsNil(o.Value) {
		return nil, false
	}
	return o.Value, true
}

// HasValue returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasValue() bool {
	if o != nil && !IsNil(o.Value) {
		return true
	}

	return false
}

// SetValue gets a reference to the given float64 and assigns it to the Value field.
func (o *TaxPolicyUpdateDto) SetValue(v float64) {
	o.Value = &v
}

// GetPercentage returns the Percentage field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetPercentage() float64 {
	if o == nil || IsNil(o.Percentage) {
		var ret float64
		return ret
	}
	return *o.Percentage
}

// GetPercentageOk returns a tuple with the Percentage field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetPercentageOk() (*float64, bool) {
	if o == nil || IsNil(o.Percentage) {
		return nil, false
	}
	return o.Percentage, true
}

// HasPercentage returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasPercentage() bool {
	if o != nil && !IsNil(o.Percentage) {
		return true
	}

	return false
}

// SetPercentage gets a reference to the given float64 and assigns it to the Percentage field.
func (o *TaxPolicyUpdateDto) SetPercentage(v float64) {
	o.Percentage = &v
}

// GetCurrencyId returns the CurrencyId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCurrencyId() string {
	if o == nil || IsNil(o.CurrencyId.Get()) {
		var ret string
		return ret
	}
	return *o.CurrencyId.Get()
}

// GetCurrencyIdOk returns a tuple with the CurrencyId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCurrencyIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CurrencyId.Get(), o.CurrencyId.IsSet()
}

// HasCurrencyId returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCurrencyId() bool {
	if o != nil && o.CurrencyId.IsSet() {
		return true
	}

	return false
}

// SetCurrencyId gets a reference to the given NullableString and assigns it to the CurrencyId field.
func (o *TaxPolicyUpdateDto) SetCurrencyId(v string) {
	o.CurrencyId.Set(&v)
}
// SetCurrencyIdNil sets the value for CurrencyId to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCurrencyIdNil() {
	o.CurrencyId.Set(nil)
}

// UnsetCurrencyId ensures that no value is present for CurrencyId, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCurrencyId() {
	o.CurrencyId.Unset()
}

// GetCountryId returns the CountryId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCountryId() string {
	if o == nil || IsNil(o.CountryId.Get()) {
		var ret string
		return ret
	}
	return *o.CountryId.Get()
}

// GetCountryIdOk returns a tuple with the CountryId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCountryIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CountryId.Get(), o.CountryId.IsSet()
}

// HasCountryId returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCountryId() bool {
	if o != nil && o.CountryId.IsSet() {
		return true
	}

	return false
}

// SetCountryId gets a reference to the given NullableString and assigns it to the CountryId field.
func (o *TaxPolicyUpdateDto) SetCountryId(v string) {
	o.CountryId.Set(&v)
}
// SetCountryIdNil sets the value for CountryId to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCountryIdNil() {
	o.CountryId.Set(nil)
}

// UnsetCountryId ensures that no value is present for CountryId, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCountryId() {
	o.CountryId.Unset()
}

// GetCountryStateId returns the CountryStateId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCountryStateId() string {
	if o == nil || IsNil(o.CountryStateId.Get()) {
		var ret string
		return ret
	}
	return *o.CountryStateId.Get()
}

// GetCountryStateIdOk returns a tuple with the CountryStateId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCountryStateIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CountryStateId.Get(), o.CountryStateId.IsSet()
}

// HasCountryStateId returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCountryStateId() bool {
	if o != nil && o.CountryStateId.IsSet() {
		return true
	}

	return false
}

// SetCountryStateId gets a reference to the given NullableString and assigns it to the CountryStateId field.
func (o *TaxPolicyUpdateDto) SetCountryStateId(v string) {
	o.CountryStateId.Set(&v)
}
// SetCountryStateIdNil sets the value for CountryStateId to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCountryStateIdNil() {
	o.CountryStateId.Set(nil)
}

// UnsetCountryStateId ensures that no value is present for CountryStateId, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCountryStateId() {
	o.CountryStateId.Unset()
}

// GetCustomState returns the CustomState field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCustomState() string {
	if o == nil || IsNil(o.CustomState.Get()) {
		var ret string
		return ret
	}
	return *o.CustomState.Get()
}

// GetCustomStateOk returns a tuple with the CustomState field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCustomStateOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CustomState.Get(), o.CustomState.IsSet()
}

// HasCustomState returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCustomState() bool {
	if o != nil && o.CustomState.IsSet() {
		return true
	}

	return false
}

// SetCustomState gets a reference to the given NullableString and assigns it to the CustomState field.
func (o *TaxPolicyUpdateDto) SetCustomState(v string) {
	o.CustomState.Set(&v)
}
// SetCustomStateNil sets the value for CustomState to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCustomStateNil() {
	o.CustomState.Set(nil)
}

// UnsetCustomState ensures that no value is present for CustomState, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCustomState() {
	o.CustomState.Unset()
}

// GetCustomCity returns the CustomCity field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCustomCity() string {
	if o == nil || IsNil(o.CustomCity.Get()) {
		var ret string
		return ret
	}
	return *o.CustomCity.Get()
}

// GetCustomCityOk returns a tuple with the CustomCity field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCustomCityOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CustomCity.Get(), o.CustomCity.IsSet()
}

// HasCustomCity returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCustomCity() bool {
	if o != nil && o.CustomCity.IsSet() {
		return true
	}

	return false
}

// SetCustomCity gets a reference to the given NullableString and assigns it to the CustomCity field.
func (o *TaxPolicyUpdateDto) SetCustomCity(v string) {
	o.CustomCity.Set(&v)
}
// SetCustomCityNil sets the value for CustomCity to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCustomCityNil() {
	o.CustomCity.Set(nil)
}

// UnsetCustomCity ensures that no value is present for CustomCity, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCustomCity() {
	o.CustomCity.Unset()
}

// GetCityId returns the CityId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetCityId() string {
	if o == nil || IsNil(o.CityId.Get()) {
		var ret string
		return ret
	}
	return *o.CityId.Get()
}

// GetCityIdOk returns a tuple with the CityId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetCityIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CityId.Get(), o.CityId.IsSet()
}

// HasCityId returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasCityId() bool {
	if o != nil && o.CityId.IsSet() {
		return true
	}

	return false
}

// SetCityId gets a reference to the given NullableString and assigns it to the CityId field.
func (o *TaxPolicyUpdateDto) SetCityId(v string) {
	o.CityId.Set(&v)
}
// SetCityIdNil sets the value for CityId to be an explicit nil
func (o *TaxPolicyUpdateDto) SetCityIdNil() {
	o.CityId.Set(nil)
}

// UnsetCityId ensures that no value is present for CityId, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetCityId() {
	o.CityId.Unset()
}

// GetZero returns the Zero field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetZero() bool {
	if o == nil || IsNil(o.Zero) {
		var ret bool
		return ret
	}
	return *o.Zero
}

// GetZeroOk returns a tuple with the Zero field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetZeroOk() (*bool, bool) {
	if o == nil || IsNil(o.Zero) {
		return nil, false
	}
	return o.Zero, true
}

// HasZero returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasZero() bool {
	if o != nil && !IsNil(o.Zero) {
		return true
	}

	return false
}

// SetZero gets a reference to the given bool and assigns it to the Zero field.
func (o *TaxPolicyUpdateDto) SetZero(v bool) {
	o.Zero = &v
}

// GetReduced returns the Reduced field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetReduced() bool {
	if o == nil || IsNil(o.Reduced) {
		var ret bool
		return ret
	}
	return *o.Reduced
}

// GetReducedOk returns a tuple with the Reduced field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetReducedOk() (*bool, bool) {
	if o == nil || IsNil(o.Reduced) {
		return nil, false
	}
	return o.Reduced, true
}

// HasReduced returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasReduced() bool {
	if o != nil && !IsNil(o.Reduced) {
		return true
	}

	return false
}

// SetReduced gets a reference to the given bool and assigns it to the Reduced field.
func (o *TaxPolicyUpdateDto) SetReduced(v bool) {
	o.Reduced = &v
}

// GetWithholding returns the Withholding field value if set, zero value otherwise.
func (o *TaxPolicyUpdateDto) GetWithholding() bool {
	if o == nil || IsNil(o.Withholding) {
		var ret bool
		return ret
	}
	return *o.Withholding
}

// GetWithholdingOk returns a tuple with the Withholding field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *TaxPolicyUpdateDto) GetWithholdingOk() (*bool, bool) {
	if o == nil || IsNil(o.Withholding) {
		return nil, false
	}
	return o.Withholding, true
}

// HasWithholding returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasWithholding() bool {
	if o != nil && !IsNil(o.Withholding) {
		return true
	}

	return false
}

// SetWithholding gets a reference to the given bool and assigns it to the Withholding field.
func (o *TaxPolicyUpdateDto) SetWithholding(v bool) {
	o.Withholding = &v
}

// GetTaxSchemeCode returns the TaxSchemeCode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetTaxSchemeCode() string {
	if o == nil || IsNil(o.TaxSchemeCode.Get()) {
		var ret string
		return ret
	}
	return *o.TaxSchemeCode.Get()
}

// GetTaxSchemeCodeOk returns a tuple with the TaxSchemeCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetTaxSchemeCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.TaxSchemeCode.Get(), o.TaxSchemeCode.IsSet()
}

// HasTaxSchemeCode returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasTaxSchemeCode() bool {
	if o != nil && o.TaxSchemeCode.IsSet() {
		return true
	}

	return false
}

// SetTaxSchemeCode gets a reference to the given NullableString and assigns it to the TaxSchemeCode field.
func (o *TaxPolicyUpdateDto) SetTaxSchemeCode(v string) {
	o.TaxSchemeCode.Set(&v)
}
// SetTaxSchemeCodeNil sets the value for TaxSchemeCode to be an explicit nil
func (o *TaxPolicyUpdateDto) SetTaxSchemeCodeNil() {
	o.TaxSchemeCode.Set(nil)
}

// UnsetTaxSchemeCode ensures that no value is present for TaxSchemeCode, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetTaxSchemeCode() {
	o.TaxSchemeCode.Unset()
}

// GetTaxCategoryCode returns the TaxCategoryCode field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetTaxCategoryCode() string {
	if o == nil || IsNil(o.TaxCategoryCode.Get()) {
		var ret string
		return ret
	}
	return *o.TaxCategoryCode.Get()
}

// GetTaxCategoryCodeOk returns a tuple with the TaxCategoryCode field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetTaxCategoryCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.TaxCategoryCode.Get(), o.TaxCategoryCode.IsSet()
}

// HasTaxCategoryCode returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasTaxCategoryCode() bool {
	if o != nil && o.TaxCategoryCode.IsSet() {
		return true
	}

	return false
}

// SetTaxCategoryCode gets a reference to the given NullableString and assigns it to the TaxCategoryCode field.
func (o *TaxPolicyUpdateDto) SetTaxCategoryCode(v string) {
	o.TaxCategoryCode.Set(&v)
}
// SetTaxCategoryCodeNil sets the value for TaxCategoryCode to be an explicit nil
func (o *TaxPolicyUpdateDto) SetTaxCategoryCodeNil() {
	o.TaxCategoryCode.Set(nil)
}

// UnsetTaxCategoryCode ensures that no value is present for TaxCategoryCode, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetTaxCategoryCode() {
	o.TaxCategoryCode.Unset()
}

// GetFiscalAuthorityId returns the FiscalAuthorityId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *TaxPolicyUpdateDto) GetFiscalAuthorityId() string {
	if o == nil || IsNil(o.FiscalAuthorityId.Get()) {
		var ret string
		return ret
	}
	return *o.FiscalAuthorityId.Get()
}

// GetFiscalAuthorityIdOk returns a tuple with the FiscalAuthorityId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *TaxPolicyUpdateDto) GetFiscalAuthorityIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FiscalAuthorityId.Get(), o.FiscalAuthorityId.IsSet()
}

// HasFiscalAuthorityId returns a boolean if a field has been set.
func (o *TaxPolicyUpdateDto) HasFiscalAuthorityId() bool {
	if o != nil && o.FiscalAuthorityId.IsSet() {
		return true
	}

	return false
}

// SetFiscalAuthorityId gets a reference to the given NullableString and assigns it to the FiscalAuthorityId field.
func (o *TaxPolicyUpdateDto) SetFiscalAuthorityId(v string) {
	o.FiscalAuthorityId.Set(&v)
}
// SetFiscalAuthorityIdNil sets the value for FiscalAuthorityId to be an explicit nil
func (o *TaxPolicyUpdateDto) SetFiscalAuthorityIdNil() {
	o.FiscalAuthorityId.Set(nil)
}

// UnsetFiscalAuthorityId ensures that no value is present for FiscalAuthorityId, not even an explicit nil
func (o *TaxPolicyUpdateDto) UnsetFiscalAuthorityId() {
	o.FiscalAuthorityId.Unset()
}

func (o TaxPolicyUpdateDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TaxPolicyUpdateDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Code.IsSet() {
		toSerialize["code"] = o.Code.Get()
	}
	if o.Title.IsSet() {
		toSerialize["title"] = o.Title.Get()
	}
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if !IsNil(o.IsFree) {
		toSerialize["isFree"] = o.IsFree
	}
	if !IsNil(o.Reduce) {
		toSerialize["reduce"] = o.Reduce
	}
	if !IsNil(o.IsEnabled) {
		toSerialize["isEnabled"] = o.IsEnabled
	}
	if !IsNil(o.IsDefault) {
		toSerialize["isDefault"] = o.IsDefault
	}
	if !IsNil(o.AllowInternational) {
		toSerialize["allowInternational"] = o.AllowInternational
	}
	if !IsNil(o.Hours) {
		toSerialize["hours"] = o.Hours
	}
	if !IsNil(o.Days) {
		toSerialize["days"] = o.Days
	}
	if !IsNil(o.Weeks) {
		toSerialize["weeks"] = o.Weeks
	}
	if !IsNil(o.Months) {
		toSerialize["months"] = o.Months
	}
	if !IsNil(o.Years) {
		toSerialize["years"] = o.Years
	}
	if !IsNil(o.Value) {
		toSerialize["value"] = o.Value
	}
	if !IsNil(o.Percentage) {
		toSerialize["percentage"] = o.Percentage
	}
	if o.CurrencyId.IsSet() {
		toSerialize["currencyId"] = o.CurrencyId.Get()
	}
	if o.CountryId.IsSet() {
		toSerialize["countryId"] = o.CountryId.Get()
	}
	if o.CountryStateId.IsSet() {
		toSerialize["countryStateId"] = o.CountryStateId.Get()
	}
	if o.CustomState.IsSet() {
		toSerialize["customState"] = o.CustomState.Get()
	}
	if o.CustomCity.IsSet() {
		toSerialize["customCity"] = o.CustomCity.Get()
	}
	if o.CityId.IsSet() {
		toSerialize["cityId"] = o.CityId.Get()
	}
	if !IsNil(o.Zero) {
		toSerialize["zero"] = o.Zero
	}
	if !IsNil(o.Reduced) {
		toSerialize["reduced"] = o.Reduced
	}
	if !IsNil(o.Withholding) {
		toSerialize["withholding"] = o.Withholding
	}
	if o.TaxSchemeCode.IsSet() {
		toSerialize["taxSchemeCode"] = o.TaxSchemeCode.Get()
	}
	if o.TaxCategoryCode.IsSet() {
		toSerialize["taxCategoryCode"] = o.TaxCategoryCode.Get()
	}
	if o.FiscalAuthorityId.IsSet() {
		toSerialize["fiscalAuthorityId"] = o.FiscalAuthorityId.Get()
	}
	return toSerialize, nil
}

type NullableTaxPolicyUpdateDto struct {
	value *TaxPolicyUpdateDto
	isSet bool
}

func (v NullableTaxPolicyUpdateDto) Get() *TaxPolicyUpdateDto {
	return v.value
}

func (v *NullableTaxPolicyUpdateDto) Set(val *TaxPolicyUpdateDto) {
	v.value = val
	v.isSet = true
}

func (v NullableTaxPolicyUpdateDto) IsSet() bool {
	return v.isSet
}

func (v *NullableTaxPolicyUpdateDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTaxPolicyUpdateDto(val *TaxPolicyUpdateDto) *NullableTaxPolicyUpdateDto {
	return &NullableTaxPolicyUpdateDto{value: val, isSet: true}
}

func (v NullableTaxPolicyUpdateDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTaxPolicyUpdateDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



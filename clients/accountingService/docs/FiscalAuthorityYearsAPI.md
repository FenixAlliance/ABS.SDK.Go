...
func main() {
	...
	// response from `PatchFiscalAuthorityYearAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `FiscalAuthorityYearsAPI.PatchFiscalAuthorityYearAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchFiscalAuthorityYearAsyncRequest struct via the builder pattern

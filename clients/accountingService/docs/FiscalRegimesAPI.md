...
[**PatchFiscalRegimeAsync**](FiscalRegimesAPI.md#PatchFiscalRegimeAsync) | **Patch** /api/v2/AccountingService/Fiscals/Authorities/FiscalRegimes/{regimeId} | Patch a fiscal regime
...
func main() {
	...
	// response from `PatchFiscalRegimeAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `FiscalRegimesAPI.PatchFiscalRegimeAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchFiscalRegimeAsyncRequest struct via the builder pattern

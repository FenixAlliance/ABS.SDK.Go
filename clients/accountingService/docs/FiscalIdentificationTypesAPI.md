...
func main() {
	...
	// response from `PatchFiscalIdentificationTypeAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `FiscalIdentificationTypesAPI.PatchFiscalIdentificationTypeAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchFiscalIdentificationTypeAsyncRequest struct via the builder pattern

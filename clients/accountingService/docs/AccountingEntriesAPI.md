...
func main() {
	...
	resp, r, err := apiClient.AccountingEntriesAPI.GetCreditsSumAsync(context.Background()).TenantId(tenantId).ApiVersion(apiVersion).XApiVersion(xApiVersion).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AccountingEntriesAPI.GetCreditsSumAsync``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetCreditsSumAsync`: DecimalEnvelope
	fmt.Fprintf(os.Stdout, "Response from `AccountingEntriesAPI.GetCreditsSumAsync`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.AccountingEntriesAPI.GetDebitsSumAsync(context.Background()).TenantId(tenantId).ApiVersion(apiVersion).XApiVersion(xApiVersion).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AccountingEntriesAPI.GetDebitsSumAsync``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetDebitsSumAsync`: DecimalEnvelope
	fmt.Fprintf(os.Stdout, "Response from `AccountingEntriesAPI.GetDebitsSumAsync`: %v\n", resp)
}
```

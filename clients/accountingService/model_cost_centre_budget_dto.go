/*
AccountingService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0.0
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
)

// checks if the CostCentreBudgetDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CostCentreBudgetDto{}

// CostCentreBudgetDto struct for CostCentreBudgetDto
type CostCentreBudgetDto struct {
	Id NullableString `json:"id,omitempty"`
	Timestamp NullableTime `json:"timestamp,omitempty"`
	Name NullableString `json:"name,omitempty"`
	TenantId NullableString `json:"tenantId,omitempty"`
	FiscalYearId NullableString `json:"fiscalYearId,omitempty"`
	CostCentreId NullableString `json:"costCentreId,omitempty"`
}

// NewCostCentreBudgetDto instantiates a new CostCentreBudgetDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCostCentreBudgetDto() *CostCentreBudgetDto {
	this := CostCentreBudgetDto{}
	return &this
}

// NewCostCentreBudgetDtoWithDefaults instantiates a new CostCentreBudgetDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCostCentreBudgetDtoWithDefaults() *CostCentreBudgetDto {
	this := CostCentreBudgetDto{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CostCentreBudgetDto) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CostCentreBudgetDto) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *CostCentreBudgetDto) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *CostCentreBudgetDto) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *CostCentreBudgetDto) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *CostCentreBudgetDto) UnsetId() {
	o.Id.Unset()
}

// GetTimestamp returns the Timestamp field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CostCentreBudgetDto) GetTimestamp() time.Time {
	if o == nil || IsNil(o.Timestamp.Get()) {
		var ret time.Time
		return ret
	}
	return *o.Timestamp.Get()
}

// GetTimestampOk returns a tuple with the Timestamp field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CostCentreBudgetDto) GetTimestampOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.Timestamp.Get(), o.Timestamp.IsSet()
}

// HasTimestamp returns a boolean if a field has been set.
func (o *CostCentreBudgetDto) HasTimestamp() bool {
	if o != nil && o.Timestamp.IsSet() {
		return true
	}

	return false
}

// SetTimestamp gets a reference to the given NullableTime and assigns it to the Timestamp field.
func (o *CostCentreBudgetDto) SetTimestamp(v time.Time) {
	o.Timestamp.Set(&v)
}
// SetTimestampNil sets the value for Timestamp to be an explicit nil
func (o *CostCentreBudgetDto) SetTimestampNil() {
	o.Timestamp.Set(nil)
}

// UnsetTimestamp ensures that no value is present for Timestamp, not even an explicit nil
func (o *CostCentreBudgetDto) UnsetTimestamp() {
	o.Timestamp.Unset()
}

// GetName returns the Name field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CostCentreBudgetDto) GetName() string {
	if o == nil || IsNil(o.Name.Get()) {
		var ret string
		return ret
	}
	return *o.Name.Get()
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CostCentreBudgetDto) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Name.Get(), o.Name.IsSet()
}

// HasName returns a boolean if a field has been set.
func (o *CostCentreBudgetDto) HasName() bool {
	if o != nil && o.Name.IsSet() {
		return true
	}

	return false
}

// SetName gets a reference to the given NullableString and assigns it to the Name field.
func (o *CostCentreBudgetDto) SetName(v string) {
	o.Name.Set(&v)
}
// SetNameNil sets the value for Name to be an explicit nil
func (o *CostCentreBudgetDto) SetNameNil() {
	o.Name.Set(nil)
}

// UnsetName ensures that no value is present for Name, not even an explicit nil
func (o *CostCentreBudgetDto) UnsetName() {
	o.Name.Unset()
}

// GetTenantId returns the TenantId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CostCentreBudgetDto) GetTenantId() string {
	if o == nil || IsNil(o.TenantId.Get()) {
		var ret string
		return ret
	}
	return *o.TenantId.Get()
}

// GetTenantIdOk returns a tuple with the TenantId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CostCentreBudgetDto) GetTenantIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.TenantId.Get(), o.TenantId.IsSet()
}

// HasTenantId returns a boolean if a field has been set.
func (o *CostCentreBudgetDto) HasTenantId() bool {
	if o != nil && o.TenantId.IsSet() {
		return true
	}

	return false
}

// SetTenantId gets a reference to the given NullableString and assigns it to the TenantId field.
func (o *CostCentreBudgetDto) SetTenantId(v string) {
	o.TenantId.Set(&v)
}
// SetTenantIdNil sets the value for TenantId to be an explicit nil
func (o *CostCentreBudgetDto) SetTenantIdNil() {
	o.TenantId.Set(nil)
}

// UnsetTenantId ensures that no value is present for TenantId, not even an explicit nil
func (o *CostCentreBudgetDto) UnsetTenantId() {
	o.TenantId.Unset()
}

// GetFiscalYearId returns the FiscalYearId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CostCentreBudgetDto) GetFiscalYearId() string {
	if o == nil || IsNil(o.FiscalYearId.Get()) {
		var ret string
		return ret
	}
	return *o.FiscalYearId.Get()
}

// GetFiscalYearIdOk returns a tuple with the FiscalYearId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CostCentreBudgetDto) GetFiscalYearIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FiscalYearId.Get(), o.FiscalYearId.IsSet()
}

// HasFiscalYearId returns a boolean if a field has been set.
func (o *CostCentreBudgetDto) HasFiscalYearId() bool {
	if o != nil && o.FiscalYearId.IsSet() {
		return true
	}

	return false
}

// SetFiscalYearId gets a reference to the given NullableString and assigns it to the FiscalYearId field.
func (o *CostCentreBudgetDto) SetFiscalYearId(v string) {
	o.FiscalYearId.Set(&v)
}
// SetFiscalYearIdNil sets the value for FiscalYearId to be an explicit nil
func (o *CostCentreBudgetDto) SetFiscalYearIdNil() {
	o.FiscalYearId.Set(nil)
}

// UnsetFiscalYearId ensures that no value is present for FiscalYearId, not even an explicit nil
func (o *CostCentreBudgetDto) UnsetFiscalYearId() {
	o.FiscalYearId.Unset()
}

// GetCostCentreId returns the CostCentreId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CostCentreBudgetDto) GetCostCentreId() string {
	if o == nil || IsNil(o.CostCentreId.Get()) {
		var ret string
		return ret
	}
	return *o.CostCentreId.Get()
}

// GetCostCentreIdOk returns a tuple with the CostCentreId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CostCentreBudgetDto) GetCostCentreIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CostCentreId.Get(), o.CostCentreId.IsSet()
}

// HasCostCentreId returns a boolean if a field has been set.
func (o *CostCentreBudgetDto) HasCostCentreId() bool {
	if o != nil && o.CostCentreId.IsSet() {
		return true
	}

	return false
}

// SetCostCentreId gets a reference to the given NullableString and assigns it to the CostCentreId field.
func (o *CostCentreBudgetDto) SetCostCentreId(v string) {
	o.CostCentreId.Set(&v)
}
// SetCostCentreIdNil sets the value for CostCentreId to be an explicit nil
func (o *CostCentreBudgetDto) SetCostCentreIdNil() {
	o.CostCentreId.Set(nil)
}

// UnsetCostCentreId ensures that no value is present for CostCentreId, not even an explicit nil
func (o *CostCentreBudgetDto) UnsetCostCentreId() {
	o.CostCentreId.Unset()
}

func (o CostCentreBudgetDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CostCentreBudgetDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if o.Timestamp.IsSet() {
		toSerialize["timestamp"] = o.Timestamp.Get()
	}
	if o.Name.IsSet() {
		toSerialize["name"] = o.Name.Get()
	}
	if o.TenantId.IsSet() {
		toSerialize["tenantId"] = o.TenantId.Get()
	}
	if o.FiscalYearId.IsSet() {
		toSerialize["fiscalYearId"] = o.FiscalYearId.Get()
	}
	if o.CostCentreId.IsSet() {
		toSerialize["costCentreId"] = o.CostCentreId.Get()
	}
	return toSerialize, nil
}

type NullableCostCentreBudgetDto struct {
	value *CostCentreBudgetDto
	isSet bool
}

func (v NullableCostCentreBudgetDto) Get() *CostCentreBudgetDto {
	return v.value
}

func (v *NullableCostCentreBudgetDto) Set(val *CostCentreBudgetDto) {
	v.value = val
	v.isSet = true
}

func (v NullableCostCentreBudgetDto) IsSet() bool {
	return v.isSet
}

func (v *NullableCostCentreBudgetDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCostCentreBudgetDto(val *CostCentreBudgetDto) *NullableCostCentreBudgetDto {
	return &NullableCostCentreBudgetDto{value: val, isSet: true}
}

func (v NullableCostCentreBudgetDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCostCentreBudgetDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



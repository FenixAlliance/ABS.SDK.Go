/*
HrmsService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0.0
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
)

// checks if the JobOfferFieldRecordDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &JobOfferFieldRecordDto{}

// JobOfferFieldRecordDto struct for JobOfferFieldRecordDto
type JobOfferFieldRecordDto struct {
	Id NullableString `json:"id,omitempty"`
	Timestamp *time.Time `json:"timestamp,omitempty"`
	JobFieldId NullableString `json:"jobFieldId,omitempty"`
	JobOfferId NullableString `json:"jobOfferId,omitempty"`
	TenantId NullableString `json:"tenantId,omitempty"`
	EnrollmentId NullableString `json:"enrollmentId,omitempty"`
}

// NewJobOfferFieldRecordDto instantiates a new JobOfferFieldRecordDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewJobOfferFieldRecordDto() *JobOfferFieldRecordDto {
	this := JobOfferFieldRecordDto{}
	return &this
}

// NewJobOfferFieldRecordDtoWithDefaults instantiates a new JobOfferFieldRecordDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewJobOfferFieldRecordDtoWithDefaults() *JobOfferFieldRecordDto {
	this := JobOfferFieldRecordDto{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JobOfferFieldRecordDto) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JobOfferFieldRecordDto) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *JobOfferFieldRecordDto) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *JobOfferFieldRecordDto) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *JobOfferFieldRecordDto) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *JobOfferFieldRecordDto) UnsetId() {
	o.Id.Unset()
}

// GetTimestamp returns the Timestamp field value if set, zero value otherwise.
func (o *JobOfferFieldRecordDto) GetTimestamp() time.Time {
	if o == nil || IsNil(o.Timestamp) {
		var ret time.Time
		return ret
	}
	return *o.Timestamp
}

// GetTimestampOk returns a tuple with the Timestamp field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *JobOfferFieldRecordDto) GetTimestampOk() (*time.Time, bool) {
	if o == nil || IsNil(o.Timestamp) {
		return nil, false
	}
	return o.Timestamp, true
}

// HasTimestamp returns a boolean if a field has been set.
func (o *JobOfferFieldRecordDto) HasTimestamp() bool {
	if o != nil && !IsNil(o.Timestamp) {
		return true
	}

	return false
}

// SetTimestamp gets a reference to the given time.Time and assigns it to the Timestamp field.
func (o *JobOfferFieldRecordDto) SetTimestamp(v time.Time) {
	o.Timestamp = &v
}

// GetJobFieldId returns the JobFieldId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JobOfferFieldRecordDto) GetJobFieldId() string {
	if o == nil || IsNil(o.JobFieldId.Get()) {
		var ret string
		return ret
	}
	return *o.JobFieldId.Get()
}

// GetJobFieldIdOk returns a tuple with the JobFieldId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JobOfferFieldRecordDto) GetJobFieldIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.JobFieldId.Get(), o.JobFieldId.IsSet()
}

// HasJobFieldId returns a boolean if a field has been set.
func (o *JobOfferFieldRecordDto) HasJobFieldId() bool {
	if o != nil && o.JobFieldId.IsSet() {
		return true
	}

	return false
}

// SetJobFieldId gets a reference to the given NullableString and assigns it to the JobFieldId field.
func (o *JobOfferFieldRecordDto) SetJobFieldId(v string) {
	o.JobFieldId.Set(&v)
}
// SetJobFieldIdNil sets the value for JobFieldId to be an explicit nil
func (o *JobOfferFieldRecordDto) SetJobFieldIdNil() {
	o.JobFieldId.Set(nil)
}

// UnsetJobFieldId ensures that no value is present for JobFieldId, not even an explicit nil
func (o *JobOfferFieldRecordDto) UnsetJobFieldId() {
	o.JobFieldId.Unset()
}

// GetJobOfferId returns the JobOfferId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JobOfferFieldRecordDto) GetJobOfferId() string {
	if o == nil || IsNil(o.JobOfferId.Get()) {
		var ret string
		return ret
	}
	return *o.JobOfferId.Get()
}

// GetJobOfferIdOk returns a tuple with the JobOfferId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JobOfferFieldRecordDto) GetJobOfferIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.JobOfferId.Get(), o.JobOfferId.IsSet()
}

// HasJobOfferId returns a boolean if a field has been set.
func (o *JobOfferFieldRecordDto) HasJobOfferId() bool {
	if o != nil && o.JobOfferId.IsSet() {
		return true
	}

	return false
}

// SetJobOfferId gets a reference to the given NullableString and assigns it to the JobOfferId field.
func (o *JobOfferFieldRecordDto) SetJobOfferId(v string) {
	o.JobOfferId.Set(&v)
}
// SetJobOfferIdNil sets the value for JobOfferId to be an explicit nil
func (o *JobOfferFieldRecordDto) SetJobOfferIdNil() {
	o.JobOfferId.Set(nil)
}

// UnsetJobOfferId ensures that no value is present for JobOfferId, not even an explicit nil
func (o *JobOfferFieldRecordDto) UnsetJobOfferId() {
	o.JobOfferId.Unset()
}

// GetTenantId returns the TenantId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JobOfferFieldRecordDto) GetTenantId() string {
	if o == nil || IsNil(o.TenantId.Get()) {
		var ret string
		return ret
	}
	return *o.TenantId.Get()
}

// GetTenantIdOk returns a tuple with the TenantId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JobOfferFieldRecordDto) GetTenantIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.TenantId.Get(), o.TenantId.IsSet()
}

// HasTenantId returns a boolean if a field has been set.
func (o *JobOfferFieldRecordDto) HasTenantId() bool {
	if o != nil && o.TenantId.IsSet() {
		return true
	}

	return false
}

// SetTenantId gets a reference to the given NullableString and assigns it to the TenantId field.
func (o *JobOfferFieldRecordDto) SetTenantId(v string) {
	o.TenantId.Set(&v)
}
// SetTenantIdNil sets the value for TenantId to be an explicit nil
func (o *JobOfferFieldRecordDto) SetTenantIdNil() {
	o.TenantId.Set(nil)
}

// UnsetTenantId ensures that no value is present for TenantId, not even an explicit nil
func (o *JobOfferFieldRecordDto) UnsetTenantId() {
	o.TenantId.Unset()
}

// GetEnrollmentId returns the EnrollmentId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *JobOfferFieldRecordDto) GetEnrollmentId() string {
	if o == nil || IsNil(o.EnrollmentId.Get()) {
		var ret string
		return ret
	}
	return *o.EnrollmentId.Get()
}

// GetEnrollmentIdOk returns a tuple with the EnrollmentId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *JobOfferFieldRecordDto) GetEnrollmentIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.EnrollmentId.Get(), o.EnrollmentId.IsSet()
}

// HasEnrollmentId returns a boolean if a field has been set.
func (o *JobOfferFieldRecordDto) HasEnrollmentId() bool {
	if o != nil && o.EnrollmentId.IsSet() {
		return true
	}

	return false
}

// SetEnrollmentId gets a reference to the given NullableString and assigns it to the EnrollmentId field.
func (o *JobOfferFieldRecordDto) SetEnrollmentId(v string) {
	o.EnrollmentId.Set(&v)
}
// SetEnrollmentIdNil sets the value for EnrollmentId to be an explicit nil
func (o *JobOfferFieldRecordDto) SetEnrollmentIdNil() {
	o.EnrollmentId.Set(nil)
}

// UnsetEnrollmentId ensures that no value is present for EnrollmentId, not even an explicit nil
func (o *JobOfferFieldRecordDto) UnsetEnrollmentId() {
	o.EnrollmentId.Unset()
}

func (o JobOfferFieldRecordDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o JobOfferFieldRecordDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if !IsNil(o.Timestamp) {
		toSerialize["timestamp"] = o.Timestamp
	}
	if o.JobFieldId.IsSet() {
		toSerialize["jobFieldId"] = o.JobFieldId.Get()
	}
	if o.JobOfferId.IsSet() {
		toSerialize["jobOfferId"] = o.JobOfferId.Get()
	}
	if o.TenantId.IsSet() {
		toSerialize["tenantId"] = o.TenantId.Get()
	}
	if o.EnrollmentId.IsSet() {
		toSerialize["enrollmentId"] = o.EnrollmentId.Get()
	}
	return toSerialize, nil
}

type NullableJobOfferFieldRecordDto struct {
	value *JobOfferFieldRecordDto
	isSet bool
}

func (v NullableJobOfferFieldRecordDto) Get() *JobOfferFieldRecordDto {
	return v.value
}

func (v *NullableJobOfferFieldRecordDto) Set(val *JobOfferFieldRecordDto) {
	v.value = val
	v.isSet = true
}

func (v NullableJobOfferFieldRecordDto) IsSet() bool {
	return v.isSet
}

func (v *NullableJobOfferFieldRecordDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableJobOfferFieldRecordDto(val *JobOfferFieldRecordDto) *NullableJobOfferFieldRecordDto {
	return &NullableJobOfferFieldRecordDto{value: val, isSet: true}
}

func (v NullableJobOfferFieldRecordDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableJobOfferFieldRecordDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



...
func main() {
	...
	// response from `PatchEmployeeTypeAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `EmployeeTypesAPI.PatchEmployeeTypeAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchEmployeeTypeAsyncRequest struct via the builder pattern

...
func main() {
	...
	// response from `PatchTrainingProgramEventAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `TrainingProgramEventsAPI.PatchTrainingProgramEventAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchTrainingProgramEventAsyncRequest struct via the builder pattern

...
func main() {
	...
	// response from `PatchSocialMediaPostAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `SocialMediaPostsAPI.PatchSocialMediaPostAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchSocialMediaPostAsyncRequest struct via the builder pattern

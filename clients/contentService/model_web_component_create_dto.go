/*
ContentService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.1.2.5532
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the WebComponentCreateDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WebComponentCreateDto{}

// WebComponentCreateDto struct for WebComponentCreateDto
type WebComponentCreateDto struct {
	Id *string `json:"id,omitempty"`
	Timestamp *time.Time `json:"timestamp,omitempty"`
	Name string `json:"name"`
	Title NullableString `json:"title,omitempty"`
	Description NullableString `json:"description,omitempty"`
	Code NullableString `json:"code,omitempty"`
	HtmlContent NullableString `json:"htmlContent,omitempty"`
	CssContent NullableString `json:"cssContent,omitempty"`
	JsContent NullableString `json:"jsContent,omitempty"`
	CodeType NullableString `json:"codeType,omitempty"`
	Published *bool `json:"published,omitempty"`
	Enable *bool `json:"enable,omitempty"`
	FeaturedImageUrl NullableString `json:"featuredImageUrl,omitempty"`
}

type _WebComponentCreateDto WebComponentCreateDto

// NewWebComponentCreateDto instantiates a new WebComponentCreateDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWebComponentCreateDto(name string) *WebComponentCreateDto {
	this := WebComponentCreateDto{}
	this.Name = name
	return &this
}

// NewWebComponentCreateDtoWithDefaults instantiates a new WebComponentCreateDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWebComponentCreateDtoWithDefaults() *WebComponentCreateDto {
	this := WebComponentCreateDto{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *WebComponentCreateDto) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebComponentCreateDto) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *WebComponentCreateDto) SetId(v string) {
	o.Id = &v
}

// GetTimestamp returns the Timestamp field value if set, zero value otherwise.
func (o *WebComponentCreateDto) GetTimestamp() time.Time {
	if o == nil || IsNil(o.Timestamp) {
		var ret time.Time
		return ret
	}
	return *o.Timestamp
}

// GetTimestampOk returns a tuple with the Timestamp field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebComponentCreateDto) GetTimestampOk() (*time.Time, bool) {
	if o == nil || IsNil(o.Timestamp) {
		return nil, false
	}
	return o.Timestamp, true
}

// HasTimestamp returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasTimestamp() bool {
	if o != nil && !IsNil(o.Timestamp) {
		return true
	}

	return false
}

// SetTimestamp gets a reference to the given time.Time and assigns it to the Timestamp field.
func (o *WebComponentCreateDto) SetTimestamp(v time.Time) {
	o.Timestamp = &v
}

// GetName returns the Name field value
func (o *WebComponentCreateDto) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *WebComponentCreateDto) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *WebComponentCreateDto) SetName(v string) {
	o.Name = v
}

// GetTitle returns the Title field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetTitle() string {
	if o == nil || IsNil(o.Title.Get()) {
		var ret string
		return ret
	}
	return *o.Title.Get()
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Title.Get(), o.Title.IsSet()
}

// HasTitle returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasTitle() bool {
	if o != nil && o.Title.IsSet() {
		return true
	}

	return false
}

// SetTitle gets a reference to the given NullableString and assigns it to the Title field.
func (o *WebComponentCreateDto) SetTitle(v string) {
	o.Title.Set(&v)
}
// SetTitleNil sets the value for Title to be an explicit nil
func (o *WebComponentCreateDto) SetTitleNil() {
	o.Title.Set(nil)
}

// UnsetTitle ensures that no value is present for Title, not even an explicit nil
func (o *WebComponentCreateDto) UnsetTitle() {
	o.Title.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *WebComponentCreateDto) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *WebComponentCreateDto) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *WebComponentCreateDto) UnsetDescription() {
	o.Description.Unset()
}

// GetCode returns the Code field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetCode() string {
	if o == nil || IsNil(o.Code.Get()) {
		var ret string
		return ret
	}
	return *o.Code.Get()
}

// GetCodeOk returns a tuple with the Code field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Code.Get(), o.Code.IsSet()
}

// HasCode returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasCode() bool {
	if o != nil && o.Code.IsSet() {
		return true
	}

	return false
}

// SetCode gets a reference to the given NullableString and assigns it to the Code field.
func (o *WebComponentCreateDto) SetCode(v string) {
	o.Code.Set(&v)
}
// SetCodeNil sets the value for Code to be an explicit nil
func (o *WebComponentCreateDto) SetCodeNil() {
	o.Code.Set(nil)
}

// UnsetCode ensures that no value is present for Code, not even an explicit nil
func (o *WebComponentCreateDto) UnsetCode() {
	o.Code.Unset()
}

// GetHtmlContent returns the HtmlContent field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetHtmlContent() string {
	if o == nil || IsNil(o.HtmlContent.Get()) {
		var ret string
		return ret
	}
	return *o.HtmlContent.Get()
}

// GetHtmlContentOk returns a tuple with the HtmlContent field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetHtmlContentOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.HtmlContent.Get(), o.HtmlContent.IsSet()
}

// HasHtmlContent returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasHtmlContent() bool {
	if o != nil && o.HtmlContent.IsSet() {
		return true
	}

	return false
}

// SetHtmlContent gets a reference to the given NullableString and assigns it to the HtmlContent field.
func (o *WebComponentCreateDto) SetHtmlContent(v string) {
	o.HtmlContent.Set(&v)
}
// SetHtmlContentNil sets the value for HtmlContent to be an explicit nil
func (o *WebComponentCreateDto) SetHtmlContentNil() {
	o.HtmlContent.Set(nil)
}

// UnsetHtmlContent ensures that no value is present for HtmlContent, not even an explicit nil
func (o *WebComponentCreateDto) UnsetHtmlContent() {
	o.HtmlContent.Unset()
}

// GetCssContent returns the CssContent field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetCssContent() string {
	if o == nil || IsNil(o.CssContent.Get()) {
		var ret string
		return ret
	}
	return *o.CssContent.Get()
}

// GetCssContentOk returns a tuple with the CssContent field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetCssContentOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CssContent.Get(), o.CssContent.IsSet()
}

// HasCssContent returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasCssContent() bool {
	if o != nil && o.CssContent.IsSet() {
		return true
	}

	return false
}

// SetCssContent gets a reference to the given NullableString and assigns it to the CssContent field.
func (o *WebComponentCreateDto) SetCssContent(v string) {
	o.CssContent.Set(&v)
}
// SetCssContentNil sets the value for CssContent to be an explicit nil
func (o *WebComponentCreateDto) SetCssContentNil() {
	o.CssContent.Set(nil)
}

// UnsetCssContent ensures that no value is present for CssContent, not even an explicit nil
func (o *WebComponentCreateDto) UnsetCssContent() {
	o.CssContent.Unset()
}

// GetJsContent returns the JsContent field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetJsContent() string {
	if o == nil || IsNil(o.JsContent.Get()) {
		var ret string
		return ret
	}
	return *o.JsContent.Get()
}

// GetJsContentOk returns a tuple with the JsContent field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetJsContentOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.JsContent.Get(), o.JsContent.IsSet()
}

// HasJsContent returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasJsContent() bool {
	if o != nil && o.JsContent.IsSet() {
		return true
	}

	return false
}

// SetJsContent gets a reference to the given NullableString and assigns it to the JsContent field.
func (o *WebComponentCreateDto) SetJsContent(v string) {
	o.JsContent.Set(&v)
}
// SetJsContentNil sets the value for JsContent to be an explicit nil
func (o *WebComponentCreateDto) SetJsContentNil() {
	o.JsContent.Set(nil)
}

// UnsetJsContent ensures that no value is present for JsContent, not even an explicit nil
func (o *WebComponentCreateDto) UnsetJsContent() {
	o.JsContent.Unset()
}

// GetCodeType returns the CodeType field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetCodeType() string {
	if o == nil || IsNil(o.CodeType.Get()) {
		var ret string
		return ret
	}
	return *o.CodeType.Get()
}

// GetCodeTypeOk returns a tuple with the CodeType field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetCodeTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CodeType.Get(), o.CodeType.IsSet()
}

// HasCodeType returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasCodeType() bool {
	if o != nil && o.CodeType.IsSet() {
		return true
	}

	return false
}

// SetCodeType gets a reference to the given NullableString and assigns it to the CodeType field.
func (o *WebComponentCreateDto) SetCodeType(v string) {
	o.CodeType.Set(&v)
}
// SetCodeTypeNil sets the value for CodeType to be an explicit nil
func (o *WebComponentCreateDto) SetCodeTypeNil() {
	o.CodeType.Set(nil)
}

// UnsetCodeType ensures that no value is present for CodeType, not even an explicit nil
func (o *WebComponentCreateDto) UnsetCodeType() {
	o.CodeType.Unset()
}

// GetPublished returns the Published field value if set, zero value otherwise.
func (o *WebComponentCreateDto) GetPublished() bool {
	if o == nil || IsNil(o.Published) {
		var ret bool
		return ret
	}
	return *o.Published
}

// GetPublishedOk returns a tuple with the Published field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebComponentCreateDto) GetPublishedOk() (*bool, bool) {
	if o == nil || IsNil(o.Published) {
		return nil, false
	}
	return o.Published, true
}

// HasPublished returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasPublished() bool {
	if o != nil && !IsNil(o.Published) {
		return true
	}

	return false
}

// SetPublished gets a reference to the given bool and assigns it to the Published field.
func (o *WebComponentCreateDto) SetPublished(v bool) {
	o.Published = &v
}

// GetEnable returns the Enable field value if set, zero value otherwise.
func (o *WebComponentCreateDto) GetEnable() bool {
	if o == nil || IsNil(o.Enable) {
		var ret bool
		return ret
	}
	return *o.Enable
}

// GetEnableOk returns a tuple with the Enable field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *WebComponentCreateDto) GetEnableOk() (*bool, bool) {
	if o == nil || IsNil(o.Enable) {
		return nil, false
	}
	return o.Enable, true
}

// HasEnable returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasEnable() bool {
	if o != nil && !IsNil(o.Enable) {
		return true
	}

	return false
}

// SetEnable gets a reference to the given bool and assigns it to the Enable field.
func (o *WebComponentCreateDto) SetEnable(v bool) {
	o.Enable = &v
}

// GetFeaturedImageUrl returns the FeaturedImageUrl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WebComponentCreateDto) GetFeaturedImageUrl() string {
	if o == nil || IsNil(o.FeaturedImageUrl.Get()) {
		var ret string
		return ret
	}
	return *o.FeaturedImageUrl.Get()
}

// GetFeaturedImageUrlOk returns a tuple with the FeaturedImageUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WebComponentCreateDto) GetFeaturedImageUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FeaturedImageUrl.Get(), o.FeaturedImageUrl.IsSet()
}

// HasFeaturedImageUrl returns a boolean if a field has been set.
func (o *WebComponentCreateDto) HasFeaturedImageUrl() bool {
	if o != nil && o.FeaturedImageUrl.IsSet() {
		return true
	}

	return false
}

// SetFeaturedImageUrl gets a reference to the given NullableString and assigns it to the FeaturedImageUrl field.
func (o *WebComponentCreateDto) SetFeaturedImageUrl(v string) {
	o.FeaturedImageUrl.Set(&v)
}
// SetFeaturedImageUrlNil sets the value for FeaturedImageUrl to be an explicit nil
func (o *WebComponentCreateDto) SetFeaturedImageUrlNil() {
	o.FeaturedImageUrl.Set(nil)
}

// UnsetFeaturedImageUrl ensures that no value is present for FeaturedImageUrl, not even an explicit nil
func (o *WebComponentCreateDto) UnsetFeaturedImageUrl() {
	o.FeaturedImageUrl.Unset()
}

func (o WebComponentCreateDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WebComponentCreateDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Timestamp) {
		toSerialize["timestamp"] = o.Timestamp
	}
	toSerialize["name"] = o.Name
	if o.Title.IsSet() {
		toSerialize["title"] = o.Title.Get()
	}
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if o.Code.IsSet() {
		toSerialize["code"] = o.Code.Get()
	}
	if o.HtmlContent.IsSet() {
		toSerialize["htmlContent"] = o.HtmlContent.Get()
	}
	if o.CssContent.IsSet() {
		toSerialize["cssContent"] = o.CssContent.Get()
	}
	if o.JsContent.IsSet() {
		toSerialize["jsContent"] = o.JsContent.Get()
	}
	if o.CodeType.IsSet() {
		toSerialize["codeType"] = o.CodeType.Get()
	}
	if !IsNil(o.Published) {
		toSerialize["published"] = o.Published
	}
	if !IsNil(o.Enable) {
		toSerialize["enable"] = o.Enable
	}
	if o.FeaturedImageUrl.IsSet() {
		toSerialize["featuredImageUrl"] = o.FeaturedImageUrl.Get()
	}
	return toSerialize, nil
}

func (o *WebComponentCreateDto) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"name",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varWebComponentCreateDto := _WebComponentCreateDto{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varWebComponentCreateDto)

	if err != nil {
		return err
	}

	*o = WebComponentCreateDto(varWebComponentCreateDto)

	return err
}

type NullableWebComponentCreateDto struct {
	value *WebComponentCreateDto
	isSet bool
}

func (v NullableWebComponentCreateDto) Get() *WebComponentCreateDto {
	return v.value
}

func (v *NullableWebComponentCreateDto) Set(val *WebComponentCreateDto) {
	v.value = val
	v.isSet = true
}

func (v NullableWebComponentCreateDto) IsSet() bool {
	return v.isSet
}

func (v *NullableWebComponentCreateDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWebComponentCreateDto(val *WebComponentCreateDto) *NullableWebComponentCreateDto {
	return &NullableWebComponentCreateDto{value: val, isSet: true}
}

func (v NullableWebComponentCreateDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWebComponentCreateDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



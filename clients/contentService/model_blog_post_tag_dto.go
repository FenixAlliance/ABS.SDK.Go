/*
ContentService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0.0
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
)

// checks if the BlogPostTagDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &BlogPostTagDto{}

// BlogPostTagDto struct for BlogPostTagDto
type BlogPostTagDto struct {
	Id NullableString `json:"id,omitempty"`
	Timestamp NullableTime `json:"timestamp,omitempty"`
	Slug NullableString `json:"slug,omitempty"`
	Type NullableString `json:"type,omitempty"`
	Title NullableString `json:"title,omitempty"`
	Description NullableString `json:"description,omitempty"`
	SeoTitle NullableString `json:"seoTitle,omitempty"`
	MetaDescription NullableString `json:"metaDescription,omitempty"`
	CornerstoneContent *bool `json:"cornerstoneContent,omitempty"`
	AllowSerachEngines *bool `json:"allowSerachEngines,omitempty"`
	SeoKeyPhrases NullableString `json:"seoKeyPhrases,omitempty"`
	CanonicalUrl NullableString `json:"canonicalUrl,omitempty"`
	ImageURL NullableString `json:"imageURL,omitempty"`
	Image NullableString `json:"image,omitempty"`
	WebPortalId NullableString `json:"webPortalId,omitempty"`
	TenantId NullableString `json:"tenantId,omitempty"`
	EnrollmentId NullableString `json:"enrollmentId,omitempty"`
}

// NewBlogPostTagDto instantiates a new BlogPostTagDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewBlogPostTagDto() *BlogPostTagDto {
	this := BlogPostTagDto{}
	return &this
}

// NewBlogPostTagDtoWithDefaults instantiates a new BlogPostTagDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewBlogPostTagDtoWithDefaults() *BlogPostTagDto {
	this := BlogPostTagDto{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetId() string {
	if o == nil || IsNil(o.Id.Get()) {
		var ret string
		return ret
	}
	return *o.Id.Get()
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Id.Get(), o.Id.IsSet()
}

// HasId returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasId() bool {
	if o != nil && o.Id.IsSet() {
		return true
	}

	return false
}

// SetId gets a reference to the given NullableString and assigns it to the Id field.
func (o *BlogPostTagDto) SetId(v string) {
	o.Id.Set(&v)
}
// SetIdNil sets the value for Id to be an explicit nil
func (o *BlogPostTagDto) SetIdNil() {
	o.Id.Set(nil)
}

// UnsetId ensures that no value is present for Id, not even an explicit nil
func (o *BlogPostTagDto) UnsetId() {
	o.Id.Unset()
}

// GetTimestamp returns the Timestamp field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetTimestamp() time.Time {
	if o == nil || IsNil(o.Timestamp.Get()) {
		var ret time.Time
		return ret
	}
	return *o.Timestamp.Get()
}

// GetTimestampOk returns a tuple with the Timestamp field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetTimestampOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.Timestamp.Get(), o.Timestamp.IsSet()
}

// HasTimestamp returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasTimestamp() bool {
	if o != nil && o.Timestamp.IsSet() {
		return true
	}

	return false
}

// SetTimestamp gets a reference to the given NullableTime and assigns it to the Timestamp field.
func (o *BlogPostTagDto) SetTimestamp(v time.Time) {
	o.Timestamp.Set(&v)
}
// SetTimestampNil sets the value for Timestamp to be an explicit nil
func (o *BlogPostTagDto) SetTimestampNil() {
	o.Timestamp.Set(nil)
}

// UnsetTimestamp ensures that no value is present for Timestamp, not even an explicit nil
func (o *BlogPostTagDto) UnsetTimestamp() {
	o.Timestamp.Unset()
}

// GetSlug returns the Slug field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetSlug() string {
	if o == nil || IsNil(o.Slug.Get()) {
		var ret string
		return ret
	}
	return *o.Slug.Get()
}

// GetSlugOk returns a tuple with the Slug field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetSlugOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Slug.Get(), o.Slug.IsSet()
}

// HasSlug returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasSlug() bool {
	if o != nil && o.Slug.IsSet() {
		return true
	}

	return false
}

// SetSlug gets a reference to the given NullableString and assigns it to the Slug field.
func (o *BlogPostTagDto) SetSlug(v string) {
	o.Slug.Set(&v)
}
// SetSlugNil sets the value for Slug to be an explicit nil
func (o *BlogPostTagDto) SetSlugNil() {
	o.Slug.Set(nil)
}

// UnsetSlug ensures that no value is present for Slug, not even an explicit nil
func (o *BlogPostTagDto) UnsetSlug() {
	o.Slug.Unset()
}

// GetType returns the Type field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetType() string {
	if o == nil || IsNil(o.Type.Get()) {
		var ret string
		return ret
	}
	return *o.Type.Get()
}

// GetTypeOk returns a tuple with the Type field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Type.Get(), o.Type.IsSet()
}

// HasType returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasType() bool {
	if o != nil && o.Type.IsSet() {
		return true
	}

	return false
}

// SetType gets a reference to the given NullableString and assigns it to the Type field.
func (o *BlogPostTagDto) SetType(v string) {
	o.Type.Set(&v)
}
// SetTypeNil sets the value for Type to be an explicit nil
func (o *BlogPostTagDto) SetTypeNil() {
	o.Type.Set(nil)
}

// UnsetType ensures that no value is present for Type, not even an explicit nil
func (o *BlogPostTagDto) UnsetType() {
	o.Type.Unset()
}

// GetTitle returns the Title field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetTitle() string {
	if o == nil || IsNil(o.Title.Get()) {
		var ret string
		return ret
	}
	return *o.Title.Get()
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Title.Get(), o.Title.IsSet()
}

// HasTitle returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasTitle() bool {
	if o != nil && o.Title.IsSet() {
		return true
	}

	return false
}

// SetTitle gets a reference to the given NullableString and assigns it to the Title field.
func (o *BlogPostTagDto) SetTitle(v string) {
	o.Title.Set(&v)
}
// SetTitleNil sets the value for Title to be an explicit nil
func (o *BlogPostTagDto) SetTitleNil() {
	o.Title.Set(nil)
}

// UnsetTitle ensures that no value is present for Title, not even an explicit nil
func (o *BlogPostTagDto) UnsetTitle() {
	o.Title.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *BlogPostTagDto) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *BlogPostTagDto) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *BlogPostTagDto) UnsetDescription() {
	o.Description.Unset()
}

// GetSeoTitle returns the SeoTitle field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetSeoTitle() string {
	if o == nil || IsNil(o.SeoTitle.Get()) {
		var ret string
		return ret
	}
	return *o.SeoTitle.Get()
}

// GetSeoTitleOk returns a tuple with the SeoTitle field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetSeoTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SeoTitle.Get(), o.SeoTitle.IsSet()
}

// HasSeoTitle returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasSeoTitle() bool {
	if o != nil && o.SeoTitle.IsSet() {
		return true
	}

	return false
}

// SetSeoTitle gets a reference to the given NullableString and assigns it to the SeoTitle field.
func (o *BlogPostTagDto) SetSeoTitle(v string) {
	o.SeoTitle.Set(&v)
}
// SetSeoTitleNil sets the value for SeoTitle to be an explicit nil
func (o *BlogPostTagDto) SetSeoTitleNil() {
	o.SeoTitle.Set(nil)
}

// UnsetSeoTitle ensures that no value is present for SeoTitle, not even an explicit nil
func (o *BlogPostTagDto) UnsetSeoTitle() {
	o.SeoTitle.Unset()
}

// GetMetaDescription returns the MetaDescription field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetMetaDescription() string {
	if o == nil || IsNil(o.MetaDescription.Get()) {
		var ret string
		return ret
	}
	return *o.MetaDescription.Get()
}

// GetMetaDescriptionOk returns a tuple with the MetaDescription field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetMetaDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.MetaDescription.Get(), o.MetaDescription.IsSet()
}

// HasMetaDescription returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasMetaDescription() bool {
	if o != nil && o.MetaDescription.IsSet() {
		return true
	}

	return false
}

// SetMetaDescription gets a reference to the given NullableString and assigns it to the MetaDescription field.
func (o *BlogPostTagDto) SetMetaDescription(v string) {
	o.MetaDescription.Set(&v)
}
// SetMetaDescriptionNil sets the value for MetaDescription to be an explicit nil
func (o *BlogPostTagDto) SetMetaDescriptionNil() {
	o.MetaDescription.Set(nil)
}

// UnsetMetaDescription ensures that no value is present for MetaDescription, not even an explicit nil
func (o *BlogPostTagDto) UnsetMetaDescription() {
	o.MetaDescription.Unset()
}

// GetCornerstoneContent returns the CornerstoneContent field value if set, zero value otherwise.
func (o *BlogPostTagDto) GetCornerstoneContent() bool {
	if o == nil || IsNil(o.CornerstoneContent) {
		var ret bool
		return ret
	}
	return *o.CornerstoneContent
}

// GetCornerstoneContentOk returns a tuple with the CornerstoneContent field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlogPostTagDto) GetCornerstoneContentOk() (*bool, bool) {
	if o == nil || IsNil(o.CornerstoneContent) {
		return nil, false
	}
	return o.CornerstoneContent, true
}

// HasCornerstoneContent returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasCornerstoneContent() bool {
	if o != nil && !IsNil(o.CornerstoneContent) {
		return true
	}

	return false
}

// SetCornerstoneContent gets a reference to the given bool and assigns it to the CornerstoneContent field.
func (o *BlogPostTagDto) SetCornerstoneContent(v bool) {
	o.CornerstoneContent = &v
}

// GetAllowSerachEngines returns the AllowSerachEngines field value if set, zero value otherwise.
func (o *BlogPostTagDto) GetAllowSerachEngines() bool {
	if o == nil || IsNil(o.AllowSerachEngines) {
		var ret bool
		return ret
	}
	return *o.AllowSerachEngines
}

// GetAllowSerachEnginesOk returns a tuple with the AllowSerachEngines field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *BlogPostTagDto) GetAllowSerachEnginesOk() (*bool, bool) {
	if o == nil || IsNil(o.AllowSerachEngines) {
		return nil, false
	}
	return o.AllowSerachEngines, true
}

// HasAllowSerachEngines returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasAllowSerachEngines() bool {
	if o != nil && !IsNil(o.AllowSerachEngines) {
		return true
	}

	return false
}

// SetAllowSerachEngines gets a reference to the given bool and assigns it to the AllowSerachEngines field.
func (o *BlogPostTagDto) SetAllowSerachEngines(v bool) {
	o.AllowSerachEngines = &v
}

// GetSeoKeyPhrases returns the SeoKeyPhrases field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetSeoKeyPhrases() string {
	if o == nil || IsNil(o.SeoKeyPhrases.Get()) {
		var ret string
		return ret
	}
	return *o.SeoKeyPhrases.Get()
}

// GetSeoKeyPhrasesOk returns a tuple with the SeoKeyPhrases field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetSeoKeyPhrasesOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.SeoKeyPhrases.Get(), o.SeoKeyPhrases.IsSet()
}

// HasSeoKeyPhrases returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasSeoKeyPhrases() bool {
	if o != nil && o.SeoKeyPhrases.IsSet() {
		return true
	}

	return false
}

// SetSeoKeyPhrases gets a reference to the given NullableString and assigns it to the SeoKeyPhrases field.
func (o *BlogPostTagDto) SetSeoKeyPhrases(v string) {
	o.SeoKeyPhrases.Set(&v)
}
// SetSeoKeyPhrasesNil sets the value for SeoKeyPhrases to be an explicit nil
func (o *BlogPostTagDto) SetSeoKeyPhrasesNil() {
	o.SeoKeyPhrases.Set(nil)
}

// UnsetSeoKeyPhrases ensures that no value is present for SeoKeyPhrases, not even an explicit nil
func (o *BlogPostTagDto) UnsetSeoKeyPhrases() {
	o.SeoKeyPhrases.Unset()
}

// GetCanonicalUrl returns the CanonicalUrl field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetCanonicalUrl() string {
	if o == nil || IsNil(o.CanonicalUrl.Get()) {
		var ret string
		return ret
	}
	return *o.CanonicalUrl.Get()
}

// GetCanonicalUrlOk returns a tuple with the CanonicalUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetCanonicalUrlOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.CanonicalUrl.Get(), o.CanonicalUrl.IsSet()
}

// HasCanonicalUrl returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasCanonicalUrl() bool {
	if o != nil && o.CanonicalUrl.IsSet() {
		return true
	}

	return false
}

// SetCanonicalUrl gets a reference to the given NullableString and assigns it to the CanonicalUrl field.
func (o *BlogPostTagDto) SetCanonicalUrl(v string) {
	o.CanonicalUrl.Set(&v)
}
// SetCanonicalUrlNil sets the value for CanonicalUrl to be an explicit nil
func (o *BlogPostTagDto) SetCanonicalUrlNil() {
	o.CanonicalUrl.Set(nil)
}

// UnsetCanonicalUrl ensures that no value is present for CanonicalUrl, not even an explicit nil
func (o *BlogPostTagDto) UnsetCanonicalUrl() {
	o.CanonicalUrl.Unset()
}

// GetImageURL returns the ImageURL field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetImageURL() string {
	if o == nil || IsNil(o.ImageURL.Get()) {
		var ret string
		return ret
	}
	return *o.ImageURL.Get()
}

// GetImageURLOk returns a tuple with the ImageURL field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetImageURLOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.ImageURL.Get(), o.ImageURL.IsSet()
}

// HasImageURL returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasImageURL() bool {
	if o != nil && o.ImageURL.IsSet() {
		return true
	}

	return false
}

// SetImageURL gets a reference to the given NullableString and assigns it to the ImageURL field.
func (o *BlogPostTagDto) SetImageURL(v string) {
	o.ImageURL.Set(&v)
}
// SetImageURLNil sets the value for ImageURL to be an explicit nil
func (o *BlogPostTagDto) SetImageURLNil() {
	o.ImageURL.Set(nil)
}

// UnsetImageURL ensures that no value is present for ImageURL, not even an explicit nil
func (o *BlogPostTagDto) UnsetImageURL() {
	o.ImageURL.Unset()
}

// GetImage returns the Image field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetImage() string {
	if o == nil || IsNil(o.Image.Get()) {
		var ret string
		return ret
	}
	return *o.Image.Get()
}

// GetImageOk returns a tuple with the Image field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetImageOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Image.Get(), o.Image.IsSet()
}

// HasImage returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasImage() bool {
	if o != nil && o.Image.IsSet() {
		return true
	}

	return false
}

// SetImage gets a reference to the given NullableString and assigns it to the Image field.
func (o *BlogPostTagDto) SetImage(v string) {
	o.Image.Set(&v)
}
// SetImageNil sets the value for Image to be an explicit nil
func (o *BlogPostTagDto) SetImageNil() {
	o.Image.Set(nil)
}

// UnsetImage ensures that no value is present for Image, not even an explicit nil
func (o *BlogPostTagDto) UnsetImage() {
	o.Image.Unset()
}

// GetWebPortalId returns the WebPortalId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetWebPortalId() string {
	if o == nil || IsNil(o.WebPortalId.Get()) {
		var ret string
		return ret
	}
	return *o.WebPortalId.Get()
}

// GetWebPortalIdOk returns a tuple with the WebPortalId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetWebPortalIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.WebPortalId.Get(), o.WebPortalId.IsSet()
}

// HasWebPortalId returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasWebPortalId() bool {
	if o != nil && o.WebPortalId.IsSet() {
		return true
	}

	return false
}

// SetWebPortalId gets a reference to the given NullableString and assigns it to the WebPortalId field.
func (o *BlogPostTagDto) SetWebPortalId(v string) {
	o.WebPortalId.Set(&v)
}
// SetWebPortalIdNil sets the value for WebPortalId to be an explicit nil
func (o *BlogPostTagDto) SetWebPortalIdNil() {
	o.WebPortalId.Set(nil)
}

// UnsetWebPortalId ensures that no value is present for WebPortalId, not even an explicit nil
func (o *BlogPostTagDto) UnsetWebPortalId() {
	o.WebPortalId.Unset()
}

// GetTenantId returns the TenantId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetTenantId() string {
	if o == nil || IsNil(o.TenantId.Get()) {
		var ret string
		return ret
	}
	return *o.TenantId.Get()
}

// GetTenantIdOk returns a tuple with the TenantId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetTenantIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.TenantId.Get(), o.TenantId.IsSet()
}

// HasTenantId returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasTenantId() bool {
	if o != nil && o.TenantId.IsSet() {
		return true
	}

	return false
}

// SetTenantId gets a reference to the given NullableString and assigns it to the TenantId field.
func (o *BlogPostTagDto) SetTenantId(v string) {
	o.TenantId.Set(&v)
}
// SetTenantIdNil sets the value for TenantId to be an explicit nil
func (o *BlogPostTagDto) SetTenantIdNil() {
	o.TenantId.Set(nil)
}

// UnsetTenantId ensures that no value is present for TenantId, not even an explicit nil
func (o *BlogPostTagDto) UnsetTenantId() {
	o.TenantId.Unset()
}

// GetEnrollmentId returns the EnrollmentId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *BlogPostTagDto) GetEnrollmentId() string {
	if o == nil || IsNil(o.EnrollmentId.Get()) {
		var ret string
		return ret
	}
	return *o.EnrollmentId.Get()
}

// GetEnrollmentIdOk returns a tuple with the EnrollmentId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *BlogPostTagDto) GetEnrollmentIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.EnrollmentId.Get(), o.EnrollmentId.IsSet()
}

// HasEnrollmentId returns a boolean if a field has been set.
func (o *BlogPostTagDto) HasEnrollmentId() bool {
	if o != nil && o.EnrollmentId.IsSet() {
		return true
	}

	return false
}

// SetEnrollmentId gets a reference to the given NullableString and assigns it to the EnrollmentId field.
func (o *BlogPostTagDto) SetEnrollmentId(v string) {
	o.EnrollmentId.Set(&v)
}
// SetEnrollmentIdNil sets the value for EnrollmentId to be an explicit nil
func (o *BlogPostTagDto) SetEnrollmentIdNil() {
	o.EnrollmentId.Set(nil)
}

// UnsetEnrollmentId ensures that no value is present for EnrollmentId, not even an explicit nil
func (o *BlogPostTagDto) UnsetEnrollmentId() {
	o.EnrollmentId.Unset()
}

func (o BlogPostTagDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o BlogPostTagDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Id.IsSet() {
		toSerialize["id"] = o.Id.Get()
	}
	if o.Timestamp.IsSet() {
		toSerialize["timestamp"] = o.Timestamp.Get()
	}
	if o.Slug.IsSet() {
		toSerialize["slug"] = o.Slug.Get()
	}
	if o.Type.IsSet() {
		toSerialize["type"] = o.Type.Get()
	}
	if o.Title.IsSet() {
		toSerialize["title"] = o.Title.Get()
	}
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if o.SeoTitle.IsSet() {
		toSerialize["seoTitle"] = o.SeoTitle.Get()
	}
	if o.MetaDescription.IsSet() {
		toSerialize["metaDescription"] = o.MetaDescription.Get()
	}
	if !IsNil(o.CornerstoneContent) {
		toSerialize["cornerstoneContent"] = o.CornerstoneContent
	}
	if !IsNil(o.AllowSerachEngines) {
		toSerialize["allowSerachEngines"] = o.AllowSerachEngines
	}
	if o.SeoKeyPhrases.IsSet() {
		toSerialize["seoKeyPhrases"] = o.SeoKeyPhrases.Get()
	}
	if o.CanonicalUrl.IsSet() {
		toSerialize["canonicalUrl"] = o.CanonicalUrl.Get()
	}
	if o.ImageURL.IsSet() {
		toSerialize["imageURL"] = o.ImageURL.Get()
	}
	if o.Image.IsSet() {
		toSerialize["image"] = o.Image.Get()
	}
	if o.WebPortalId.IsSet() {
		toSerialize["webPortalId"] = o.WebPortalId.Get()
	}
	if o.TenantId.IsSet() {
		toSerialize["tenantId"] = o.TenantId.Get()
	}
	if o.EnrollmentId.IsSet() {
		toSerialize["enrollmentId"] = o.EnrollmentId.Get()
	}
	return toSerialize, nil
}

type NullableBlogPostTagDto struct {
	value *BlogPostTagDto
	isSet bool
}

func (v NullableBlogPostTagDto) Get() *BlogPostTagDto {
	return v.value
}

func (v *NullableBlogPostTagDto) Set(val *BlogPostTagDto) {
	v.value = val
	v.isSet = true
}

func (v NullableBlogPostTagDto) IsSet() bool {
	return v.isSet
}

func (v *NullableBlogPostTagDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableBlogPostTagDto(val *BlogPostTagDto) *NullableBlogPostTagDto {
	return &NullableBlogPostTagDto{value: val, isSet: true}
}

func (v NullableBlogPostTagDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableBlogPostTagDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



...
func main() {
	...
	// response from `PatchMaintenanceVisitAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `MaintenanceVisitsAPI.PatchMaintenanceVisitAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchMaintenanceVisitAsyncRequest struct via the builder pattern

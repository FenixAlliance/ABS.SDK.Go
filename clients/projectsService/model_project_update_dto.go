/*
ProjectsService

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 2.0.0.0
Contact: support@fenix-alliance.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
	"time"
)

// checks if the ProjectUpdateDto type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ProjectUpdateDto{}

// ProjectUpdateDto struct for ProjectUpdateDto
type ProjectUpdateDto struct {
	Title NullableString `json:"title,omitempty"`
	Description NullableString `json:"description,omitempty"`
	IndividualId NullableString `json:"individualId,omitempty"`
	OrganizationId NullableString `json:"organizationId,omitempty"`
	ProjectStartDate *time.Time `json:"projectStartDate,omitempty"`
	ProjectEndDate *time.Time `json:"projectEndDate,omitempty"`
}

// NewProjectUpdateDto instantiates a new ProjectUpdateDto object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewProjectUpdateDto() *ProjectUpdateDto {
	this := ProjectUpdateDto{}
	return &this
}

// NewProjectUpdateDtoWithDefaults instantiates a new ProjectUpdateDto object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewProjectUpdateDtoWithDefaults() *ProjectUpdateDto {
	this := ProjectUpdateDto{}
	return &this
}

// GetTitle returns the Title field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProjectUpdateDto) GetTitle() string {
	if o == nil || IsNil(o.Title.Get()) {
		var ret string
		return ret
	}
	return *o.Title.Get()
}

// GetTitleOk returns a tuple with the Title field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProjectUpdateDto) GetTitleOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Title.Get(), o.Title.IsSet()
}

// HasTitle returns a boolean if a field has been set.
func (o *ProjectUpdateDto) HasTitle() bool {
	if o != nil && o.Title.IsSet() {
		return true
	}

	return false
}

// SetTitle gets a reference to the given NullableString and assigns it to the Title field.
func (o *ProjectUpdateDto) SetTitle(v string) {
	o.Title.Set(&v)
}
// SetTitleNil sets the value for Title to be an explicit nil
func (o *ProjectUpdateDto) SetTitleNil() {
	o.Title.Set(nil)
}

// UnsetTitle ensures that no value is present for Title, not even an explicit nil
func (o *ProjectUpdateDto) UnsetTitle() {
	o.Title.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProjectUpdateDto) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProjectUpdateDto) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *ProjectUpdateDto) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *ProjectUpdateDto) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *ProjectUpdateDto) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *ProjectUpdateDto) UnsetDescription() {
	o.Description.Unset()
}

// GetIndividualId returns the IndividualId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProjectUpdateDto) GetIndividualId() string {
	if o == nil || IsNil(o.IndividualId.Get()) {
		var ret string
		return ret
	}
	return *o.IndividualId.Get()
}

// GetIndividualIdOk returns a tuple with the IndividualId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProjectUpdateDto) GetIndividualIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.IndividualId.Get(), o.IndividualId.IsSet()
}

// HasIndividualId returns a boolean if a field has been set.
func (o *ProjectUpdateDto) HasIndividualId() bool {
	if o != nil && o.IndividualId.IsSet() {
		return true
	}

	return false
}

// SetIndividualId gets a reference to the given NullableString and assigns it to the IndividualId field.
func (o *ProjectUpdateDto) SetIndividualId(v string) {
	o.IndividualId.Set(&v)
}
// SetIndividualIdNil sets the value for IndividualId to be an explicit nil
func (o *ProjectUpdateDto) SetIndividualIdNil() {
	o.IndividualId.Set(nil)
}

// UnsetIndividualId ensures that no value is present for IndividualId, not even an explicit nil
func (o *ProjectUpdateDto) UnsetIndividualId() {
	o.IndividualId.Unset()
}

// GetOrganizationId returns the OrganizationId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ProjectUpdateDto) GetOrganizationId() string {
	if o == nil || IsNil(o.OrganizationId.Get()) {
		var ret string
		return ret
	}
	return *o.OrganizationId.Get()
}

// GetOrganizationIdOk returns a tuple with the OrganizationId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ProjectUpdateDto) GetOrganizationIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.OrganizationId.Get(), o.OrganizationId.IsSet()
}

// HasOrganizationId returns a boolean if a field has been set.
func (o *ProjectUpdateDto) HasOrganizationId() bool {
	if o != nil && o.OrganizationId.IsSet() {
		return true
	}

	return false
}

// SetOrganizationId gets a reference to the given NullableString and assigns it to the OrganizationId field.
func (o *ProjectUpdateDto) SetOrganizationId(v string) {
	o.OrganizationId.Set(&v)
}
// SetOrganizationIdNil sets the value for OrganizationId to be an explicit nil
func (o *ProjectUpdateDto) SetOrganizationIdNil() {
	o.OrganizationId.Set(nil)
}

// UnsetOrganizationId ensures that no value is present for OrganizationId, not even an explicit nil
func (o *ProjectUpdateDto) UnsetOrganizationId() {
	o.OrganizationId.Unset()
}

// GetProjectStartDate returns the ProjectStartDate field value if set, zero value otherwise.
func (o *ProjectUpdateDto) GetProjectStartDate() time.Time {
	if o == nil || IsNil(o.ProjectStartDate) {
		var ret time.Time
		return ret
	}
	return *o.ProjectStartDate
}

// GetProjectStartDateOk returns a tuple with the ProjectStartDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProjectUpdateDto) GetProjectStartDateOk() (*time.Time, bool) {
	if o == nil || IsNil(o.ProjectStartDate) {
		return nil, false
	}
	return o.ProjectStartDate, true
}

// HasProjectStartDate returns a boolean if a field has been set.
func (o *ProjectUpdateDto) HasProjectStartDate() bool {
	if o != nil && !IsNil(o.ProjectStartDate) {
		return true
	}

	return false
}

// SetProjectStartDate gets a reference to the given time.Time and assigns it to the ProjectStartDate field.
func (o *ProjectUpdateDto) SetProjectStartDate(v time.Time) {
	o.ProjectStartDate = &v
}

// GetProjectEndDate returns the ProjectEndDate field value if set, zero value otherwise.
func (o *ProjectUpdateDto) GetProjectEndDate() time.Time {
	if o == nil || IsNil(o.ProjectEndDate) {
		var ret time.Time
		return ret
	}
	return *o.ProjectEndDate
}

// GetProjectEndDateOk returns a tuple with the ProjectEndDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProjectUpdateDto) GetProjectEndDateOk() (*time.Time, bool) {
	if o == nil || IsNil(o.ProjectEndDate) {
		return nil, false
	}
	return o.ProjectEndDate, true
}

// HasProjectEndDate returns a boolean if a field has been set.
func (o *ProjectUpdateDto) HasProjectEndDate() bool {
	if o != nil && !IsNil(o.ProjectEndDate) {
		return true
	}

	return false
}

// SetProjectEndDate gets a reference to the given time.Time and assigns it to the ProjectEndDate field.
func (o *ProjectUpdateDto) SetProjectEndDate(v time.Time) {
	o.ProjectEndDate = &v
}

func (o ProjectUpdateDto) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ProjectUpdateDto) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.Title.IsSet() {
		toSerialize["title"] = o.Title.Get()
	}
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if o.IndividualId.IsSet() {
		toSerialize["individualId"] = o.IndividualId.Get()
	}
	if o.OrganizationId.IsSet() {
		toSerialize["organizationId"] = o.OrganizationId.Get()
	}
	if !IsNil(o.ProjectStartDate) {
		toSerialize["projectStartDate"] = o.ProjectStartDate
	}
	if !IsNil(o.ProjectEndDate) {
		toSerialize["projectEndDate"] = o.ProjectEndDate
	}
	return toSerialize, nil
}

type NullableProjectUpdateDto struct {
	value *ProjectUpdateDto
	isSet bool
}

func (v NullableProjectUpdateDto) Get() *ProjectUpdateDto {
	return v.value
}

func (v *NullableProjectUpdateDto) Set(val *ProjectUpdateDto) {
	v.value = val
	v.isSet = true
}

func (v NullableProjectUpdateDto) IsSet() bool {
	return v.isSet
}

func (v *NullableProjectUpdateDto) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProjectUpdateDto(val *ProjectUpdateDto) *NullableProjectUpdateDto {
	return &NullableProjectUpdateDto{value: val, isSet: true}
}

func (v NullableProjectUpdateDto) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProjectUpdateDto) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}



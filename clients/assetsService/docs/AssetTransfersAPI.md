...
func main() {
	...
	// response from `PatchAssetTransferAsync`: EmptyEnvelope
	fmt.Fprintf(os.Stdout, "Response from `AssetTransfersAPI.PatchAssetTransferAsync`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiPatchAssetTransferAsyncRequest struct via the builder pattern
